(()=>{
  ...
  function renderAccount(){
    ...
    if(session){
      ...
      host.innerHTML=`<div class="profilePreview accountProfilePreview presence-${mode}"><span class="accountDrawerAvatar"><img class="avatar" src="${session.avatar||'assets/images/sos-logo.png'}"><i></i></span><div><strong>${session.displayName||session.email}</strong><p>${session.rankName||session.rank_name||session.role||'member'}</p><small>${stateLabel}${statusText?` — ${statusText}`:""}</small></div></div><nav class="accountActionGridV417"><a class="accountFeatureActionV417" href="members.html"><i>👤</i><span><strong>Profile</strong><small>Manage your public member profile</small></span></a>${(session.role==="admin"||session.collaborationAccess)?'<a class="accountFeatureActionV417" href="collaboration.html"><i>🤝</i><span><strong>Collaboration Studio</strong><small>Projects, messages and versioned notes</small></span></a>':''}${session.role==="admin"?'<a class="accountFeatureActionV417" href="admin.html"><i>🎛</i><span><strong>Admin Hub</strong><small>Manage website content and members</small></span></a>':''}${['admin','administrator','owner','premium_member','developer'].includes(String(session.rankName||session.rank_name||session.role||'').toLowerCase().replace(/\s+/g,'_'))?'<a class="accountFeatureActionV417" href="video-effects-studio.html"><i>🎬</i><span><strong>Video Effects Studio</strong><small>Waveforms, trimming and beat-reactive rendering</small></span></a>':''}<button class="accountFeatureActionV417 accountLogoutV417" id="quickLogout" type="button"><i>🚪</i><span><strong>Sign Out</strong><small>Safely end this session</small></span></button></nav>`;
    }else{
