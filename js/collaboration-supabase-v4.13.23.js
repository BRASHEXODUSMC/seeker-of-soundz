...
async function fetchEligibleProfiles(){
 ...
 profiles=new Map((Array.isArray(data)?data:[]).map(p=>[p.id,p]));
 return profiles;
}
...
async function openProjectModal(invitedId=''){
 try{await fetchEligibleProfiles()}catch(error){console.warn('Could not refresh eligible collaborators',error)}
 populateProjectModal(invitedId);
 const modal=$('#collabProjectModal');
 const form=$('#newCollabProjectForm');
 form?.reset();
 populateProjectModal(invitedId);
 if(invitedId&&form?.elements?.collaboratorId)form.elements.collaboratorId.value=invitedId;
 modal.hidden=false;
...
async function load(){
 const [{data:ps,error:pe},{data:pr,error:pre},{data:pm,error:pme}]=await Promise.all([
  client.rpc('collaboration_list_eligible_members'),
 ...
 profiles=new Map((Array.isArray(ps)?ps:[]).map(p=>[p.id,p]));projects=pr||[];membersByProject=new Map();
...
function ensureProgressPanel(){
 const inspector=document.querySelector('.collabInspector')||document.querySelector('.collabRightPanel')||document.querySelector('#collabApp aside:last-of-type');
 if(!inspector||document.getElementById('collabProgressPanel'))return;
 const panel=document.createElement('section');
 panel.className='collabProgressPanel';
 panel.id='collabProgressPanel';
 panel.innerHTML=`
   <div class="collabProgressHead">
     <div><p class="sectionEyebrow">Completion</p><h3>Project progression</h3></div>
     <strong id="collabProgressValue">0%</strong>
   </div>
   <div class="collabProgressTrack"><i id="collabProgressFill"></i></div>
   <input id="collabProgressRange" type="range" min="0" max="100" step="1" value="0" aria-label="Project completion percentage">
 ...
 const anchor=document.getElementById('projectMembers')?.closest('section')||document.getElementById('projectMembers')?.parentElement;
 if(anchor?.parentElement)anchor.insertAdjacentElement('afterend',panel);else inspector.appendChild(panel);
 panel.querySelector('#collabProgressRange')?.addEventListener('input',e=>{
   const value=Math.max(0,Math.min(100,Number(e.target.value)||0));
   updateProgressVisual(value);
 });
 panel.querySelector('#collabProgressRange')?.addEventListener('change',e=>{
   const value=Math.max(0,Math.min(100,Number(e.target.value)||0));
   const nearest=stages.reduce((best,item)=>Math.abs(item[1]-value)<Math.abs(best[1]-value)?item:best,stages[0]);
   void updateProject({progress:value,stage:nearest[0]});
 });
}
function updateProgressVisual(value){
 const val=document.getElementById('collabProgressValue');
 ...
 const milestones=document.getElementById('collabProgressMilestones');
 if(val)val.textContent=`${value}%`;
 if(fill)fill.style.width=`${value}%`;
 if(range&&document.activeElement!==range)range.value=String(value);
 const nearest=stages.reduce((best,item)=>Math.abs(item[1]-value)<Math.abs(best[1]-value)?item:best,stages[0]);
 if(state)state.textContent=nearest[0];
 if(milestones)milestones.innerHTML=stages.filter(([,p])=>[0,20,45,70,90,100].includes(p)).map(([name,p])=>`<button type="button" class="${value>=p?'reached':''}" data-progress-jump="${p}" data-stage-jump="${esc(name)}"><span>${p}%</span><small>${esc(name)}</small></button>`).join('');
 milestones?.querySelectorAll('[data-progress-jump]').forEach(btn=>btn.addEventListener('click',()=>void updateProject({progress:Number(btn.dataset.progressJump),stage:btn.dataset.stageJump})));
}

function renderInspector(){
 ensureProgressPanel();
 updateProgressVisual(Number(selected?.progress||0));
...
function bind(){
 ensureProjectModal();
 $('#newProjectButton')?.addEventListener('click',()=>{void openProjectModal()});
 ...
 document.addEventListener('click',async e=>{
  ...
  b=e.target.closest('[data-open-project-modal]');if(b){void openProjectModal();return}
  b=e.target.closest('[data-project]');if(b){selected=projects.find(p=>p.id===b.dataset.project);await loadMessages();renderAll();return}
  b=e.target.closest('[data-new-with]');if(b){void openProjectModal(b.dataset.newWith);return}
...
async function boot(){if(!await auth())return;const notice=app.querySelector('.backendNotice');if(notice)notice.innerHTML='<strong>Supabase workspace</strong><br>Projects, members, private messages, notes, stages, and audio files are stored securely in Supabase.';bind();try{await load()}catch(e){console.error(e);toast(e.message,'Collaboration could not load')}const channel=client.channel(`collab-${user.id}`)
 .on('postgres_changes',{event:'*',schema:'public',table:'collaboration_messages'},async()=>{await loadMessages();renderConversation()})
 .on('postgres_changes',{event:'*',schema:'public',table:'collaboration_projects'},load)
 .on('postgres_changes',{event:'UPDATE',schema:'public',table:'profiles'},async()=>{try{await fetchEligibleProfiles();renderAll()}catch(error){console.warn(error)}})
 .subscribe();
 const refreshEligible=async()=>{if(document.visibilityState==='visible'){try{await fetchEligibleProfiles();populateProjectModal()}catch(error){console.warn(error)}}};
 window.addEventListener('focus',refreshEligible);
 document.addEventListener('visibilitychange',refreshEligible);
 window.addEventListener('beforeunload',()=>{client.removeChannel(channel);window.removeEventListener('focus',refreshEligible);document.removeEventListener('visibilitychange',refreshEligible)},{once:true})}
