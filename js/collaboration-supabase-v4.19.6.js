...
const isProjectMember=p=>Boolean(p&&(p.created_by===user.id||(membersByProject.get(p.id)||[]).some(m=>m.user_id===user.id)||['owner','administrator'].includes(profile?.role)));
...
function renderComposerState(){
 const form=$('#messageForm'),textarea=form?.elements?.message,send=form?.querySelector('button[type="submit"]'),file=$('#messageFile');
 if(!form||!textarea||!send)return;
 const allowed=Boolean(selected&&isProjectMember(selected));
 textarea.disabled=!allowed;
 send.disabled=!allowed;
 if(file)file.disabled=!allowed;
 textarea.placeholder=!selected?'Choose a project before writing a reply…':allowed?'Write feedback, production notes, or a private message…':'You must be added to this project before replying.';
 form.classList.toggle('isComposerLockedV4196',!allowed);
}
...
function renderAll(){renderContacts();renderProjects();renderConversation();renderInspector();renderComposerState();}
