...
const musicVolume=$('#composerMusicVolumeV4180'),sourceVolume=$('#composerSourceVolumeV4180');
...
async function playPreview(){
 ...
 await Promise.all([video.play(),audio.play()]);cancelAnimationFrame(raf);draw();setStatus('Preview playing. Open Effects to change presets; selections update live.');
}
...
audioInput.addEventListener('change',()=>{
 const file=audioInput.files[0];if(!file)return;revoke(audioUrl);audioUrl=URL.createObjectURL(file);audio.src=audioUrl;audio.load();$('#composerAudioNameV4180').textContent=file.name;setStatus('Music loaded. Preview or render the project.');
});
