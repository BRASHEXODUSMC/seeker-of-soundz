...
let logoImage=null,logoUrl='',liveDrawing=false;
const logoInput=$('#producerLogoInputV4200');
const logoEnabled=$('#producerLogoEnabledV4200');
const logoPosition=$('#producerLogoPositionV4200');
const logoSize=$('#producerLogoSizeV4200');
const logoOpacity=$('#producerLogoOpacityV4200');
const overlayText=$('#producerOverlayTextV4200');
const textStyle=$('#producerTextStyleV4200');
const textPosition=$('#producerTextPositionV4200');
const textSize=$('#producerTextSizeV4200');
const textOpacity=$('#producerTextOpacityV4200');
...
function ensureAudioGraph(){
 ...
 try{
  ...
  videoNode.connect(videoGain);videoGain.connect(analyser);
 }catch(error){console.warn('[Producer Hub] Video audio graph unavailable',error)}
 if(audio.src){
  try{
   audioNode=audioContext.createMediaElementSource(audio);
   musicGain=audioContext.createGain();
   audioNode.connect(musicGain);musicGain.connect(analyser);
  }catch(error){console.warn('[Producer Hub] Music audio graph unavailable',error)}
 }
...
function overlay(w,h,lv){
 ...
 if(fx.includes('dropFlash')&&lv.bass>.72){const flash=ctx2d.createRadialGradient(w/2,h/2,0,w/2,h/2,Math.max(w,h)*.7);flash.addColorStop(0,`rgba(255,255,255,${lv.bass*.62})`);flash.addColorStop(1,'rgba(175,70,255,0)');ctx2d.fillStyle=flash;ctx2d.fillRect(0,0,w,h)}
 if(fx.includes('vhs')){ctx2d.save();ctx2d.globalAlpha=.16;for(let i=0;i<7;i++){const y=((time*95+i*137)%h);ctx2d.fillStyle=i%2?'#fff':'#7136a8';ctx2d.fillRect(Math.sin(time*4+i)*12,y,w,1+Math.random()*3)}ctx2d.restore()}
 if(fx.includes('filmGrain'))staticNoise(w,h,.18+lv.high*.2);
 if(fx.includes('snowNoise')){ctx2d.save();ctx2d.fillStyle='rgba(255,255,255,.42)';for(let i=0;i<90;i++){const x=(i*97+time*71)%w,y=(i*53+time*(20+i%7))%h;ctx2d.fillRect(x,y,1.5,1.5)}ctx2d.restore()}
 if(fx.includes('glitchBars')||fx.includes('dataMosh')){ctx2d.save();ctx2d.globalCompositeOperation='screen';for(let i=0;i<6;i++){const y=Math.random()*h,bh=2+Math.random()*18,dx=(Math.random()-.5)*35*lv.peak;ctx2d.drawImage(canvas,0,y,w,bh,dx,y,w,bh)}ctx2d.restore()}
 ...
 if(fx.includes('oscilloscope')){ctx2d.save();ctx2d.strokeStyle='rgba(115,245,205,.72)';ctx2d.lineWidth=2;ctx2d.beginPath();for(let x=0;x<w;x+=4){const y=h*.5+Math.sin(x*.035+time*8)*h*(.02+lv.mid*.09);x?ctx2d.lineTo(x,y):ctx2d.moveTo(x,y)}ctx2d.stroke();ctx2d.restore()}
 if(fx.includes('lightLeaks')){const leak=ctx2d.createLinearGradient(0,0,w,h);leak.addColorStop(0,`rgba(255,84,174,${.04+lv.mid*.1})`);leak.addColorStop(.5,'rgba(255,190,80,0)');leak.addColorStop(1,`rgba(104,57,255,${.04+lv.high*.09})`);ctx2d.fillStyle=leak;ctx2d.fillRect(0,0,w,h)}
 if(fx.includes('horizonGlow')){const hg=ctx2d.createLinearGradient(0,h*.55,0,h);hg.addColorStop(0,'rgba(160,70,255,0)');hg.addColorStop(.65,`rgba(160,70,255,${.08+lv.bass*.18})`);hg.addColorStop(1,'rgba(0,0,0,.32)');ctx2d.fillStyle=hg;ctx2d.fillRect(0,h*.45,w,h*.55)}
 ...
 if(fx.includes('digitalRain')){ctx2d.save();ctx2d.fillStyle='rgba(115,245,205,.38)';ctx2d.font=`${Math.max(10,w*.012)}px monospace`;for(let i=0;i<28;i++){const x=i*w/28,y=(time*(40+i%5)*8+i*73)%h;ctx2d.fillText(String.fromCharCode(48+(i*7)%42),x,y)}ctx2d.restore()}
 ...
 drawCustomLayers(w,h,lv);
}
function drawCustomLayers(w,h,lv){
 if(logoImage&&logoEnabled?.checked){
  const fraction=Number(logoSize?.value||.2),opacity=Number(logoOpacity?.value||.9);
  const targetW=w*fraction,targetH=targetW*(logoImage.naturalHeight/logoImage.naturalWidth);
  const pad=w*.025,pos=logoPosition?.value||'bottom-right';
  let x=pad,y=pad;
  if(pos.includes('right'))x=w-targetW-pad;
  if(pos.includes('bottom'))y=h-targetH-pad;
  if(pos==='center'){x=(w-targetW)/2;y=(h-targetH)/2}
  ctx2d.save();ctx2d.globalAlpha=opacity;ctx2d.shadowColor='rgba(190,110,255,.55)';ctx2d.shadowBlur=12+lv.peak*18;ctx2d.drawImage(logoImage,x,y,targetW,targetH);ctx2d.restore();
 }
 const text=String(overlayText?.value||'').trim();
 if(text){
  ...
  const fonts={retro:'italic 900',arcade:'900',cyber:'800',rave:'900',minimal:'700',terminal:'700'};
  const families={retro:'Trebuchet MS, sans-serif',arcade:'Courier New, monospace',cyber:'Arial Black, sans-serif',rave:'Arial Black, sans-serif',minimal:'Arial, sans-serif',terminal:'Consolas, monospace'};
  ctx2d.save();ctx2d.globalAlpha=opacity;ctx2d.textAlign='center';ctx2d.textBaseline='middle';ctx2d.font=`${fonts[style]||'800'} ${size}px ${families[style]||'sans-serif'}`;
  let y=pos==='top'?h*.12:pos==='center'?h*.5:pos==='lower-third'?h*.68:h*.86;
  if(style==='retro'){ctx2d.shadowColor='#ff48bb';ctx2d.shadowBlur=16;ctx2d.strokeStyle='#4eeaff';ctx2d.lineWidth=Math.max(2,size*.055);ctx2d.strokeText(text,w/2,y);ctx2d.fillStyle='#fff'}
  else if(style==='arcade'){ctx2d.shadowColor='#9d4eff';ctx2d.shadowBlur=0;ctx2d.fillStyle='#fff'}
  else if(style==='cyber'){ctx2d.shadowColor='#8d4bff';ctx2d.shadowBlur=18;ctx2d.fillStyle='#d9b6ff'}
  else if(style==='rave'){ctx2d.shadowColor=`hsla(${(video.currentTime*60)%360},100%,65%,.9)`;ctx2d.shadowBlur=24;ctx2d.fillStyle='#fff'}
  else if(style==='terminal'){ctx2d.shadowColor='#53f7bd';ctx2d.shadowBlur=12;ctx2d.fillStyle='#83ffd1'}
  else{ctx2d.shadowColor='rgba(0,0,0,.8)';ctx2d.shadowBlur=8;ctx2d.fillStyle='#fff'}
  ctx2d.fillText(text,w/2,y);ctx2d.restore();
 }
}
function draw(){
...
async function playPreview(){
 ...
 syncVolumes();cancelAnimationFrame(raf);draw();
 await video.play();
 if(audio.src)await audio.play().catch(()=>{});
 updateAudioStatus(audioInput.files[0]?.name||'Video effects preview',audio.src?'Synchronized with the single video preview.':'Effects are previewing with the original video audio.',true);
 setStatus('Preview playing. Asset effects, text, and logos update live.');
}
async function render(){
 ...
 await video.play();if(audio.src)await audio.play().catch(()=>{});cancelAnimationFrame(raf);draw();recorder.start(250);setStatus('Rendering video with selected effects, text, logo, and available audio. Keep this tab open.');
...
function drawStill(){
 ...
 if(video.paused)setTimeout(()=>cancelAnimationFrame(raf),80);
}
document.addEventListener('change',event=>{
 if(event.target.matches('[data-effect],#effectStrengthV4171,#producerLogoEnabledV4200,#producerLogoPositionV4200,#producerLogoSizeV4200,#producerLogoOpacityV4200,#producerOverlayTextV4200,#producerTextStyleV4200,#producerTextPositionV4200,#producerTextSizeV4200,#producerTextOpacityV4200'))drawStill();
});
document.addEventListener('input',event=>{
 if(event.target.matches('#effectStrengthV4171,#producerLogoSizeV4200,#producerLogoOpacityV4200,#producerOverlayTextV4200,#producerTextSizeV4200,#producerTextOpacityV4200'))drawStill();
});
logoInput?.addEventListener('change',()=>{
 const file=logoInput.files?.[0];if(!file)return;
 if(logoUrl)URL.revokeObjectURL(logoUrl);logoUrl=URL.createObjectURL(file);
 const image=new Image();image.onload=()=>{logoImage=image;$('#producerLogoNameV4200').textContent=file.name;drawStill()};image.src=logoUrl;
});
$('#clearProducerLogoV4200')?.addEventListener('click',()=>{logoImage=null;if(logoUrl)URL.revokeObjectURL(logoUrl);logoUrl='';if(logoInput)logoInput.value='';$('#producerLogoNameV4200').textContent='PNG, JPG, WEBP, or GIF';drawStill()});
$('#clearProducerTextV4200')?.addEventListener('click',()=>{if(overlayText)overlayText.value='';drawStill()});
video.addEventListener('play',()=>{cancelAnimationFrame(raf);draw()});
video.addEventListener('pause',()=>{setTimeout(()=>cancelAnimationFrame(raf),60)});
video.addEventListener('seeked',drawStill);

videoInput.addEventListener('change',()=>{
 const file=videoInput.files[0];if(!file)return;revoke(videoUrl);videoUrl=URL.createObjectURL(file);video.src=videoUrl;video.load();$('#composerVideoNameV4180').textContent=file.name;setStatus('Video loaded. Effects, text, and logo layers can preview immediately. Licensed music is optional.');
});
audioInput.addEventListener('change',()=>{
 ...
 if(audioContext&&!audioNode){
  try{audioNode=audioContext.createMediaElementSource(audio);musicGain=audioContext.createGain();audioNode.connect(musicGain);musicGain.connect(analyser);syncVolumes()}catch(error){console.warn('[Producer Hub] Could not attach uploaded music',error)}
 }
 $('#composerAudioNameV4180').textContent=file.name;updateAudioStatus(file.name,'This music is mixed into the single video preview and finished export.',true);setStatus('Music loaded. Preview or render the project.');ready();
});
