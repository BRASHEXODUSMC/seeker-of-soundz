...
function cameraTransform(w,h,lv,time){
 const mode=cameraMotion?.value||'none',strength=Number(cameraStrength?.value||.7);
...
function draw(){
 ...
 const lv=levels(),fx=selectedEffects(),p=effectStrength(),time=video.currentTime||0,state=transitionState(time,video.duration||0),motion=cameraTransform(w,h,lv,time);
 ...
 ctx2d.drawImage(video,0,0,w,h);ctx2d.restore();overlay(w,h,lv);window.SOSProducerLayerManagerV4230?.draw?.(ctx2d,w,h,time,lv,video.duration||0);applyTransitionOverlay(w,h,state);
...
async function render(){
 ...
 const monitor=()=>{
  if(!rendering)return;
  const progress=Math.max(0,Math.min(1,(video.currentTime-start)/(end-start))),percent=Math.round(progress*100);
