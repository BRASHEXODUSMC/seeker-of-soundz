...
function draw(){
 ...
 ctx2d.drawImage(video,0,0,w,h);ctx2d.restore();overlay(w,h,lv);window.SOSProducerLayerManagerV4230?.draw?.(ctx2d,w,h,time,lv,projectDuration);applyTransitionOverlay(w,h,state);
 raf=requestAnimationFrame(draw);
}
async function playPreview(){
 ensureAudioGraph();if(audioContext)await audioContext.resume();
 const start=Math.max(0,Number(startInput.value||0));
 if(window.SOSVideoClipsV4240?.hasMultiple?.())await window.SOSVideoClipsV4240.startSequence(start);
 else if(window.SOSVideoClipsV4240?.hasMultiple?.())await window.SOSVideoClipsV4240.startSequence(start,true);
