...
function draw(){
 ...
 if(musicGain){
  ...
  musicGain.gain.value=razorActive&&trimActive?keyedVolume*duck:0;
  const mapped=window.SOSMediaSettingsV4330?.mapMusicTime?.(time);
  if(Number.isFinite(mapped)&&audio.src&&!audio.paused&&Math.abs(audio.currentTime-mapped)>.22)audio.currentTime=mapped;
 }
