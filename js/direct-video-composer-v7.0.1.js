...
function ensureAudioGraph(){
 ...
 if(!audioContext){
  ...
  }catch(error){console.warn('[Producer Hub 7.0.1] Video audio graph',error)}
  analyser.connect(audioContext.destination);
  analyser.connect(mediaDestination);
 }
 if(audio.src&&!audioNode){
  try{
   audioNode=audioContext.createMediaElementSource(audio);
   musicGain=audioContext.createGain();
   audioNode.connect(musicGain);musicGain.connect(analyser);
  }catch(error){console.warn('[Producer Hub 7.0.1] Music audio graph',error)}
 }
 audio.muted=false;audio.defaultMuted=false;audio.volume=1;
 syncVolumes();
}
function syncVolumes(){
 const sourceValue=Math.max(0,Math.min(1.5,Number(sourceVolume.value||1)));
 const musicValue=Math.max(0,Math.min(1.5,Number(musicVolume.value||.8)));
 if(videoGain)videoGain.gain.value=sourceValue;
 if(musicGain)musicGain.gain.value=musicValue;
 audio.muted=false;audio.defaultMuted=false;audio.volume=1;
}
function levels(){
...
function drawLayers(w,h,lv){
 ...
 const time=window.SOSVideoClipsV4240?.globalTime?.()??Number(video.currentTime||0);
 const style=textStyle?.value||'retro',size=Number(textSize?.value||54),opacity=window.SOSProSuiteV4320?.evaluate?.('textOpacity',time,Number(textOpacity?.value||.95))??Number(textOpacity?.value||.95),pos=textPosition?.value||'bottom';
 ...
 const [baseXp,baseYp]=textCoordinates(w,h,pos);
 const xp=(window.SOSProSuiteV4320?.evaluate?.('textX',time,baseXp*100)??baseXp*100)/100;
 const yp=(window.SOSProSuiteV4320?.evaluate?.('textY',time,baseYp*100)??baseYp*100)/100;
 const x=w*xp,y=h*yp,spacing=Number(textSpacing?.value||2);
 const textRotationValue=window.SOSProSuiteV4320?.evaluate?.('textRotation',time,Number(textRotation?.value||0))??Number(textRotation?.value||0);
 ctx2d.save();ctx2d.translate(x,y);ctx2d.rotate(textRotationValue*Math.PI/180);ctx2d.globalAlpha=opacity;ctx2d.textAlign='center';ctx2d.textBaseline='middle';ctx2d.font=`900 ${size}px ${families[style]||'sans-serif'}`;
...
function cameraTransform(w,h,lv,time){
 const director=window.SOSProSuiteV4320?.directorState?.(time)||null;
 const automatic=director||window.SOSAutoCameraV4250?.state?.(time)||null;
 ...
 if(mode==='director-push'){const p=Math.sin((automatic?.progress||0)*Math.PI/2);scale=1+p*.12*strength}
 if(mode==='director-pull'){const p=Math.sin((automatic?.progress||0)*Math.PI/2);scale=1.12-p*.12*strength}
 if(mode==='director-pan-left'){dx=-w*.05*Math.sin((automatic?.progress||0)*Math.PI)*strength;scale=1.04}
 if(mode==='director-pan-right'){dx=w*.05*Math.sin((automatic?.progress||0)*Math.PI)*strength;scale=1.04}
 if(mode==='director-drift'){dx=Math.sin(time*.6)*w*.025*strength;dy=Math.cos(time*.45)*h*.018*strength;scale=1.035}
 if(mode==='director-impact'){const p=Math.sin((automatic?.progress||0)*Math.PI);scale=1+p*.15*strength;dx=(Math.random()-.5)*p*28*strength;dy=(Math.random()-.5)*p*20*strength}
 if(mode==='director-shake'){const p=Math.sin((automatic?.progress||0)*Math.PI);dx=(Math.random()-.5)*p*54*strength;dy=(Math.random()-.5)*p*42*strength;rotation=(Math.random()-.5)*p*.04*strength;scale=1.04}
 if(mode==='director-spin'){const p=Math.sin((automatic?.progress||0)*Math.PI);rotation=p*.1*strength;scale=1+p*.07}
 scale=window.SOSProSuiteV4320?.evaluate?.('cameraScale',time,scale)??scale;
 rotation=window.SOSProSuiteV4320?.evaluate?.('cameraRotation',time,rotation)??rotation;
 dx=window.SOSProSuiteV4320?.evaluate?.('cameraX',time,dx)??dx;
 dy=window.SOSProSuiteV4320?.evaluate?.('cameraY',time,dy)??dy;
...
function drawVideoFrameV4330(ctx,source,w,h){
 const settings=window.SOSMediaSettingsV4330?.video?.()||{mode:'stretch'};
 const sw=source.videoWidth||w,sh=source.videoHeight||h;
 if(settings.mode==='stretch'||!sw||!sh){ctx.drawImage(source,0,0,w,h);return}
 const scale=settings.mode==='fill'?Math.max(w/sw,h/sh):Math.min(w/sw,h/sh);
 const dw=sw*scale,dh=sh*scale,dx=(w-dw)/2,dy=(h-dh)/2;
 ctx.fillStyle='#000';ctx.fillRect(0,0,w,h);ctx.drawImage(source,dx,dy,dw,dh);
}
...
function drawProjectVideoV4300(w,h,time,lv){
 ...
 if(distortion==='none'||pulse<=.001){
  drawVideoFrameV4330(ctx2d,video,w,h);
  return;
 }
 if(distortion==='warp'){
  const slices=32;
 ...
 if(distortion==='lens'){
  ...
  drawVideoFrameV4330(ctx2d,video,w,h);ctx2d.restore();ctx2d.filter='none';
  return;
 }
 if(distortion==='twist'){
  const bands=20;
...
function draw(){
 ...
 if(musicGain){
  const razorActive=window.SOSMusicRazorV4270?.isActive?.(time)!==false;
  const trimActive=window.SOSMediaSettingsV4330?.musicActive?.(time)!==false;
  const keyedVolume=window.SOSProSuiteV4320?.evaluate?.('musicVolume',time,Number(musicVolume.value))??Number(musicVolume.value);
  const duck=window.SOSProSuiteV4320?.duckGain?.(lv)??1;
  const shouldPlay=razorActive&&trimActive;
  const studioGain=Math.max(.05,Number(window.SOSProfessionalSuiteV700?.musicGain?.()??1));
  const trackAudible=window.SOSProfessionalSuiteV700?.isTrackAudible?.('music')!==false;
  musicGain.gain.value=shouldPlay&&trackAudible?Math.max(0,keyedVolume*duck*studioGain):0;
  const mapped=window.SOSMediaSettingsV4330?.mapMusicTime?.(time);
  if(Number.isFinite(mapped)&&audio.src){
   if(shouldPlay){
    if(Math.abs(audio.currentTime-mapped)>.22)audio.currentTime=mapped;
 ...
 const studioFilter=window.SOSProfessionalSuiteV700?.videoFilter?.()||'';
 ctx2d.filter=`${fx.includes('monochrome')?'grayscale(1) ':fx.includes('posterize')?'contrast(1.5) saturate(1.7) ':''}${blur?`blur(${blur}px) `:''}${studioFilter}`.trim()||'none';
 ctx2d.globalCompositeOperation=window.SOSProfessionalSuiteV700?.blend?.()||'source-over';
 drawProjectVideoV4300(w,h,time,lv);ctx2d.globalCompositeOperation='source-over';ctx2d.restore();
 const studioVignette=Number(window.SOSProfessionalSuiteV700?.vignette?.()||0);
 if(studioVignette>0){const g=ctx2d.createRadialGradient(w/2,h/2,Math.min(w,h)*.18,w/2,h/2,Math.max(w,h)*.72);g.addColorStop(0,'rgba(0,0,0,0)');g.addColorStop(1,`rgba(0,0,0,${Math.min(.9,studioVignette)})`);ctx2d.fillStyle=g;ctx2d.fillRect(0,0,w,h)}
 const studioMask=window.SOSProfessionalSuiteV700?.mask?.()||'none';
 if(studioMask==='cinema'){ctx2d.fillStyle='#000';ctx2d.fillRect(0,0,w,h*.1);ctx2d.fillRect(0,h*.9,w,h*.1)}
 if(studioMask==='spotlight'){ctx2d.save();ctx2d.fillStyle='rgba(0,0,0,.68)';ctx2d.fillRect(0,0,w,h);ctx2d.globalCompositeOperation='destination-out';ctx2d.beginPath();ctx2d.arc(w/2,h/2,Math.min(w,h)*.28,0,Math.PI*2);ctx2d.fill();ctx2d.restore()}
 overlay(w,h,lv);window.SOSProducerLayerManagerV4230?.draw?.(ctx2d,w,h,time,lv,projectDuration);applyTransitionOverlay(w,h,state);window.SOSClipTransitionsV4250?.draw?.(ctx2d,w,h,time);
...
audioInput.addEventListener('change',()=>{
 ...
 revoke(audioUrl);audioUrl=URL.createObjectURL(file);
 audio.pause();audio.src=audioUrl;audio.muted=false;audio.defaultMuted=false;audio.volume=1;audio.load();
 ensureAudioGraph();
...
window.SOSProducerAudioV701={
 context:()=>audioContext,
 ensure:ensureAudioGraph,
 resume:async()=>{ensureAudioGraph();if(audioContext?.state==='suspended')await audioContext.resume();syncVolumes();return audioContext?.state},
 setMusicGain:value=>{ensureAudioGraph();if(musicGain)musicGain.gain.value=Math.max(0,Number(value)||0)},
 sync:syncVolumes,
 audio:()=>audio
};
