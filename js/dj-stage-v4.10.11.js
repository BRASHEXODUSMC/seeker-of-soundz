...
const defaults={mode:'line',color:'violet',lightPreset:'arena',laserPreset:'festival',visual:'1',laserSpeed:'1',laserBrightness:'.8',lights:true,spots:true,strobe:true,lasers:true,crowd:true,focus:true};
...
function updateState(){
 const playing=!!activeAudio&&!activeAudio.paused&&!activeAudio.ended;stage.classList.toggle('is-playing',playing);controls.play.textContent=playing?'Pause':'Play';controls.play.setAttribute('aria-pressed',String(playing));stateEl.textContent=playing?'EDM concert visualization live':'Waiting for website audio';document.documentElement.classList.toggle('dj-audio-live',playing);
 const focus=playing&&settings.focus&&!isPerformance();document.body.classList.toggle('dj-show-live',focus);
 if(!playing)document.body.classList.remove('dj-show-live');
}
function apply(){
 ...
 controls.mode.value=settings.mode;controls.color.value=settings.color;controls.lightPreset.value=settings.lightPreset;controls.laserPreset.value=settings.laserPreset;controls.visual.value=settings.visual;controls.laserSpeed.value=settings.laserSpeed;controls.laserBrightness.value=settings.laserBrightness;updateState();
}
...
window.addEventListener('resize',resize,{passive:true});document.addEventListener('visibilitychange',updateState);window.addEventListener('beforeunload',()=>{cancelAnimationFrame(raf);document.body.classList.remove('dj-show-live')});
