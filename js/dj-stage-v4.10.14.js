...
let activeAudio=null,audioContext=null,analyser=null,dataArray=null,frequencyArray=null,raf=0,last=0,queue=[],queueRenderSignature='',beatAverage=.18,lastBeatAt=0,beatTimer=0;
...
const defaults={mode:'line',color:'violet',lightPreset:'arena',laserPreset:'festival',visual:'1',laserSpeed:'1',laserBrightness:'.8',lights:true,spots:true,strobe:true,strobeSync:'beat',lasers:true,crowd:true,focus:true,playbackMode:'stop'};
...
function pulseStrobeFromBeat(level,t){
 if(!stage.classList.contains('is-playing')||!settings.strobe||isPerformance())return;
 const mode=settings.strobeSync||'beat';
 if(mode==='pattern')return;
 beatAverage=beatAverage*.92+level*.08;
 const threshold=Math.max(.22,beatAverage*1.22);
 const cooldown=300;
 if(level<threshold||t-lastBeatAt<cooldown)return;
 lastBeatAt=t;
 const strength=Math.min(1,Math.max(.35,(level-threshold)*4+.45));
 stage.style.setProperty('--dj-beat-energy',strength.toFixed(3));
 stage.classList.remove('dj-beat-flash');
 void stage.offsetWidth;
 stage.classList.add('dj-beat-flash');
 clearTimeout(beatTimer);
 beatTimer=setTimeout(()=>stage.classList.remove('dj-beat-flash'),110);
}
...
function draw(t){
 ...
 pulseStrobeFromBeat(e,t);stage.style.setProperty('--dj-live-energy',e.toFixed(3));ctx.clearRect(0,0,w,h);const grad=ctx.createLinearGradient(0,0,w,0);grad.addColorStop(0,c1+'22');grad.addColorStop(.5,c2);grad.addColorStop(1,c1+'22');ctx.strokeStyle=grad;ctx.fillStyle=grad;ctx.lineWidth=1.8+intensity*.7;ctx.shadowBlur=10*intensity;ctx.shadowColor=c1;
...
function scan(){
 document.querySelectorAll('audio').forEach(registerAudio);
 renderQueue();
}
...
const audioObserver=new MutationObserver(mutations=>{
 ...
 for(const mutation of mutations){
  ...
  if(target instanceof Element&&(target.closest('#djTrackQueue')||target.closest('.djShowPortal')))continue;
  for(const node of mutation.addedNodes){
   if(node.nodeType!==1)continue;
   const element=node;
   if(element.matches?.('audio')||element.querySelector?.('audio')){shouldScan=true;break}
  }
  if(shouldScan)break;
 }
...
[['mode','mode'],['color','color'],['strobeSync','strobeSync'],['lightPreset','lightPreset'],['laserPreset','laserPreset'],['visual','visual'],['laserSpeed','laserSpeed'],['laserBrightness','laserBrightness']].forEach(([id,key])=>controls[id]?.addEventListener('input',()=>{settings[key]=controls[id].value;save();apply()}));
...
window.addEventListener('resize',resize,{passive:true});document.addEventListener('visibilitychange',updateState);window.addEventListener('beforeunload',()=>{cancelAnimationFrame(raf);clearTimeout(beatTimer);audioObserver.disconnect();exitFocus()});
