...
document.addEventListener("change",e=>{const input=e.target.closest("[data-experience-studio] [data-setting]");if(!input)return;const st=input.closest("[data-experience-studio]"),draft=collectStudioSettings(st);updatePreview(draft,input.dataset.setting==="transition",st);markStudioDirty(st)});
