...
const defaults={starfield:"balanced",nebula:false,shootingStars:true,loader:"signal",loaderSwipe:false,loaderProgressStyle:"bar",loaderShowLogo:true,loaderShowOrbits:true,loaderShowBrand:true,loaderShowSubtitle:true,loaderShowStatus:true,loaderShowPercent:true,loaderLogoMotion:"pulse",loaderBackgroundStyle:"cosmic",loaderBackgroundMotion:"drift",loaderLogoSpeed:"normal",motion:"balanced",cursorGlow:true,theme:"original",transition:"stellar",contrast:"balanced",cardGlow:true};
...
function applySettings(settings){
 ...
 h.classList.toggle("sos-nebula-enabled",!!s.nebula);h.classList.toggle("sos-shooting-disabled",!s.shootingStars);h.classList.toggle("sos-cursor-glow",!!s.cursorGlow);h.classList.toggle("sos-card-glow",!!s.cardGlow);h.classList.toggle("sos-loader-swipe-enabled",!!s.loaderSwipe);["bar","waveform","dual","minimal"].forEach(style=>h.classList.toggle(`sos-loader-progress-${style}`,s.loaderProgressStyle===style));h.classList.toggle("sos-loader-hide-logo",!s.loaderShowLogo);h.classList.toggle("sos-loader-hide-orbits",!s.loaderShowOrbits);h.classList.toggle("sos-loader-hide-brand",!s.loaderShowBrand);h.classList.toggle("sos-loader-hide-subtitle",!s.loaderShowSubtitle);h.classList.toggle("sos-loader-hide-status",!s.loaderShowStatus);h.classList.toggle("sos-loader-hide-percent",!s.loaderShowPercent);
...
function updatePreview(s=currentSettings(),autoPlay=false,scope=document){scope.querySelectorAll('[data-experience-live-preview]').forEach(p=>{const changed=p.dataset.transition&&p.dataset.transition!==s.transition;p.dataset.theme=s.theme;p.dataset.transition=s.transition;p.dataset.motion=s.motion;p.dataset.stars=s.starfield;p.classList.toggle('hasNebula',!!s.nebula);p.classList.toggle('hasGlow',!!s.cardGlow);const label=p.querySelector('[data-preview-transition-name]');if(label)label.textContent=transitionNames[s.transition]||'Page Transition';if(autoPlay||changed)playPreview(p)});scope.querySelectorAll('[data-loader-profile-preview]').forEach(p=>{p.dataset.progressStyle=s.loaderProgressStyle||'bar';p.classList.toggle('hideLogo',!s.loaderShowLogo);p.classList.toggle('hideOrbits',!s.loaderShowOrbits);p.classList.toggle('hideBrand',!s.loaderShowBrand);p.classList.toggle('hideSubtitle',!s.loaderShowSubtitle);p.classList.toggle('hideStatus',!s.loaderShowStatus);p.classList.toggle('hidePercent',!s.loaderShowPercent);p.classList.toggle('showSwipe',!!s.loaderSwipe);p.dataset.logoMotion=s.loaderLogoMotion||"pulse";p.dataset.logoSpeed=s.loaderLogoSpeed||"normal";p.dataset.backgroundStyle=s.loaderBackgroundStyle||"cosmic";p.dataset.backgroundMotion=s.loaderBackgroundMotion||"drift"})}
...
function experienceToast(message,title="Profile Settings"){
 if(window.SOS?.toast){
  window.SOS.toast(message,{title,icon:"✓",duration:4200});
  return;
 }
 let host=document.getElementById("experienceToastHostV4194");
 if(!host){
  host=document.createElement("div");
  host.id="experienceToastHostV4194";
  host.className="experienceToastHostV4194";
  document.body.appendChild(host);
 }
 const toast=document.createElement("div");
 toast.className="experienceToastV4194";
 toast.innerHTML=`<i>✓</i><div><strong>${title}</strong><span>${message}</span></div>`;
 host.appendChild(toast);
 requestAnimationFrame(()=>toast.classList.add("isVisible"));
 setTimeout(()=>{toast.classList.remove("isVisible");setTimeout(()=>toast.remove(),260)},4200);
}
