...
function fromElement(el){
 ...
 return{id:el.dataset.profileId||'',name,avatar,role:el.dataset.profileRole||'member',rank:el.dataset.profileRank||'New Listener',reputation:Number(el.dataset.profileReputation||0),bio:el.dataset.profileBio||'Community member on the Seeker Of SoundZ frequency.',location:el.dataset.profileLocation||'Not shared'};
}
...
function html(p){const c=counts(p.name);const href=p.id?`members.html?profile=${encodeURIComponent(p.id)}`:'members.html';return `<div class="profileHoverGlow"></div><div class="profileHoverHead"><img src="${esc(p.avatar)}" alt="${esc(p.name)}"><div><p class="profileHoverEyebrow">Community frequency</p><h3>${esc(p.name)}</h3><div class="profileHoverBadges"><span class="roleBadge">${esc(roleLabel(p.role))}</span><span class="rankBadge">${esc(p.rank)}</span></div></div></div><p class="profileHoverBio">${esc(p.bio)}</p><div class="profileHoverLocation">📍 ${esc(p.location)}</div><div class="profileMiniStats"><div><strong>${c.topic}</strong><span>Topics here</span></div><div><strong>${c.replies}</strong><span>Replies here</span></div><div><strong>${p.reputation}</strong><span>Reputation</span></div></div><a class="primaryButton profileHoverOpen" href="${href}">Open Member Profile →</a>`}
...
document.addEventListener('click',e=>{const el=e.target.closest('[data-profile-name]');if(!el)return;const id=el.dataset.profileId;if(id){e.preventDefault();location.href=`members.html?profile=${encodeURIComponent(id)}`}});
