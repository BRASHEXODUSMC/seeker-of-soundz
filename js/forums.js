...
const REPLY_REACTIONS=Object.freeze([['heart','❤️','Love'],['fire','🔥','Fire'],['like','👍','Like'],['laugh','😂','Funny'],['music','🎵','Music'],['support','🙌','Support']]);
...
function syncAchievementCompatibility(){
 try{
  if(!window.SOS?.write||!window.SOS?.K)return;
  const profileName=id=>display(state.profiles.get(id));
  const legacyPosts=state.topics.map(topic=>({
   id:topic.id,
   authorId:topic.author_id,
   author:profileName(topic.author_id),
   title:topic.title,
   body:topic.body||'',
   ...
   mediaUrl:topic.media_url||'',
   date:topic.created_at,
   likes:state.reactions.filter(r=>r.topic_id===topic.id).length,
   replies:(state.replies.get(topic.id)||[]).map(reply=>({
    id:reply.id,
    authorId:reply.author_id,
    author:profileName(reply.author_id),
    body:reply.body||'',
    date:reply.created_at,
    reactions:state.reactions.filter(r=>r.reply_id===reply.id).map(r=>r.reaction)
   }))
  }));
  window.SOS.write(window.SOS.K.posts,legacyPosts);
  const existing=window.SOS.read?.(window.SOS.K.users,[])||[];
  const byId=new Map(existing.map(u=>[u.id,u]));
  state.profiles.forEach(profile=>{
   const prior=byId.get(profile.id)||{};
   byId.set(profile.id,{...prior,id:profile.id,displayName:display(profile),username:profile.username||'',avatar:profile.avatar_url||prior.avatar||'',bio:profile.biography||prior.bio||'',location:profile.location||prior.location||'',role:String(profile.role||'member').toLowerCase()==='administrator'?'admin':String(profile.role||'member').toLowerCase(),joinedAt:profile.created_at||prior.joinedAt||new Date().toISOString()});
  });
  window.SOS.write(window.SOS.K.users,[...byId.values()]);
  window.dispatchEvent(new CustomEvent('sos:forum-data-synced',{detail:{topics:legacyPosts.length,replies:[...state.replies.values()].flat().length,reactions:state.reactions.length}}));
 }catch(error){console.warn('Achievement compatibility sync skipped:',error)}
}
...
function replyReactionCount(id,type='heart'){return state.reactions.filter(r=>r.reply_id===id&&r.reaction===type).length}
function replyReacted(id,type='heart'){return !!state.user&&state.reactions.some(r=>r.reply_id===id&&r.user_id===state.user.id&&r.reaction===type)}
function replyReactionBar(replyId){return `<div class="replyReactionBar" aria-label="Reply reactions"><span class="replyReactionHint">React</span>${REPLY_REACTIONS.map(([type,emoji,label])=>`<button class="replyReactionButton ${replyReacted(replyId,type)?'is-reacted':''}" type="button" data-reply-reaction="${type}" data-reply-id="${replyId}" title="${label}" aria-label="${label}"><span class="replyReactionEmoji">${emoji}</span><span class="replyReactionCount">${replyReactionCount(replyId,type)}</span></button>`).join('')}</div>`}
