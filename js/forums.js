...
function confirmForumDelete(kind,id,button){
 const isReply=kind==='reply';
 if(!window.SOS?.toast)return;
 window.SOS.toast(isReply?'Delete this reply permanently?':'Delete this discussion and all replies permanently?',{
  ...
  onAction:async()=>{
   ...
   if(q.error){if(button)button.disabled=false;return window.SOS.toast(q.error.message,{title:'Delete failed'})}
   if(isReply){
   ...
   render();
   await refresh();
   window.SOS.toast(isReply?'The reply was deleted.':'The discussion was deleted.',{title:'Forums'});
  }
 });
 setTimeout(()=>document.querySelector('.toastStack .appToast')?.classList.add('toastConfirm'),0);
}
