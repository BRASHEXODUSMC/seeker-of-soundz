...
function bind(){
 ...
 $('#postList')?.addEventListener('click',async e=>{const tag=e.target.closest('[data-tag]');if(tag){state.tag=tag.dataset.tag;render();return}const reply=e.target.closest('[data-reply]');if(reply){if(!state.user)return toast('Please sign in to reply.','Members only');state.replying=state.replying===reply.dataset.reply?'':reply.dataset.reply;render();return}if(e.target.closest('[data-cancel-reply]')){state.replying='';render();return}const like=e.target.closest('[data-like]');if(like){if(!state.user)return toast('Please sign in to react.','Members only');const q=await supa().rpc('forum_toggle_reaction',{target_topic:like.dataset.like,target_reply:null,reaction_value:'heart'});if(q.error)return toast(q.error.message,'Reaction');await refresh();return}const replyLike=e.target.closest('[data-like-reply]');if(replyLike){if(!state.user)return toast('Please sign in to react.','Members only');const q=await supa().rpc('forum_toggle_reaction',{target_topic:null,target_reply:replyLike.dataset.likeReply,reaction_value:'heart'});if(q.error)return toast(q.error.message,'Reaction');await refresh();return}const pin=e.target.closest('[data-pin]');if(pin){const t=state.topics.find(x=>x.id===pin.dataset.pin);const {error}=await supa().rpc('forum_set_topic_moderation',{target_topic:t.id,pin_value:!t.is_pinned,lock_value:null});if(error)return toast(error.message,'Moderation');await refresh();return}const lock=e.target.closest('[data-lock]');if(lock){const t=state.topics.find(x=>x.id===lock.dataset.lock);const {error}=await supa().rpc('forum_set_topic_moderation',{target_topic:t.id,pin_value:null,lock_value:!t.is_locked});if(error)return toast(error.message,'Moderation');await refresh();return}const del=e.target.closest('[data-delete]');if(del&&confirm('Delete this discussion and all replies?')){del.disabled=true;const {error}=await supa().rpc('forum_delete_topic',{target_topic:del.dataset.delete});if(error){del.disabled=false;return toast(error.message,'Delete')}state.topics=state.topics.filter(t=>t.id!==del.dataset.delete);render();await refresh();toast('The discussion was deleted.','Forums');return}const dr=e.target.closest('[data-delete-reply]');if(dr&&confirm('Delete this reply?')){dr.disabled=true;const {error}=await supa().rpc('forum_delete_reply',{target_reply:dr.dataset.deleteReply});if(error){dr.disabled=false;return toast(error.message,'Delete')}await refresh();toast('The reply was deleted.','Forums')}});
 $('#postList')?.addEventListener('submit',async e=>{const form=e.target.closest('[data-reply-form]');if(!form)return;e.preventDefault();const body=form.querySelector('textarea').value.trim();if(!body)return;const submit=form.querySelector('button[type="submit"]');if(submit)submit.disabled=true;const {data,error}=await supa().rpc('forum_create_reply',{target_topic:form.dataset.replyForm,reply_body:body,parent_reply:null,quoted_reply:null});if(error){if(submit)submit.disabled=false;return toast(error.message,'Reply')}state.replying='';await refresh();toast('Your reply is now live.','Reply posted')});
}
