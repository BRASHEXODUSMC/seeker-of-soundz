...
function expandedMarkers(){
 const audioDuration=Math.max(.01,buffer?.duration||projectDuration());
 const total=Math.max(audioDuration,projectDuration());
 if(total<=audioDuration+.05)return markers.map(row=>({...row}));
 const rows=[];
 for(let offset=0;offset<total;offset+=audioDuration){
  markers.forEach(marker=>{
   const time=marker.time+offset;
   if(time<=total)rows.push({...marker,time,loop:offset>0});
  });
  if(rows.length>1800)break;
 }
 return rows.slice(0,1800);
}
function render(){
 const duration=Math.max(.01,projectDuration(),buffer?.duration||0);
 const displayMarkers=expandedMarkers();
 markerLane.innerHTML=displayMarkers.map((marker,index)=>{
  const drop=marker.drop||index%16===0;
  return `<button type="button" class="beatMarkerV4190 ${drop?'isDrop':''}" style="left:${marker.time/duration*100}%" title="${formatTime(marker.time)}${drop?' · strong beat':''}${marker.loop?' · repeated music cycle':''}" data-marker-time="${marker.time}" data-beat-energy="${marker.energy}" aria-label="Beat at ${formatTime(marker.time)}"></button>`;
 }).join('');
 const status=$('#timelineStatusV4190');
 if(status)status.textContent=displayMarkers.length?`${displayMarkers.length} beat markers across the full project`:'No beat markers detected';
 ...
 window.SOSBeatAnalysisV4270={markers:()=>displayMarkers.map(row=>({...row})),sourceMarkers:()=>markers.map(row=>({...row})),buffer:()=>buffer,duration:()=>duration};
 window.dispatchEvent(new CustomEvent('sos:beat-analysis-complete',{detail:{count:displayMarkers.length,markers:displayMarkers.map(row=>({...row})),duration}}));
