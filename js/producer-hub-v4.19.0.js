...
function updateTransport(){
 $('#producerCurrentTimeV4190').textContent=formatTime(video?.currentTime||0);
 $('#producerDurationV4190').textContent=formatTime(video?.duration||0);
 const duration=video?.duration||1,progress=Math.min(1,(video?.currentTime||0)/duration);
 playhead.style.left=`calc(120px + (100% - 120px) * ${progress})`;
...
$('#producerRestartV4190')?.addEventListener('click',()=>{if(video)video.currentTime=0;if(audio)audio.currentTime=0});
