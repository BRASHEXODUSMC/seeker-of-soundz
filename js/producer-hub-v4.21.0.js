...
let theatre=false,dragging=false,decodedBuffer=null,decodedName='';
...
function duration(){return Math.max(Number.isFinite(video?.duration)?video.duration:0,0)}
function seekFromPointer(e,target){
 const d=duration();if(!d)return;const r=target.getBoundingClientRect(),x=Math.max(0,Math.min(r.width,e.clientX-r.left));video.currentTime=d*x/Math.max(1,r.width);
...
if(playhead){
 ...
 playhead.addEventListener('pointermove',e=>{if(dragging)seekFromPointer(e,tracks||playhead.parentElement)});
 playhead.addEventListener('pointerup',e=>{dragging=false;playhead.releasePointerCapture?.(e.pointerId)});
 playhead.addEventListener('keydown',e=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(e.key))return;e.preventDefault();const d=duration(),step=e.shiftKey?5:.5;if(e.key==='Home')video.currentTime=0;else if(e.key==='End')video.currentTime=d;else video.currentTime=Math.max(0,Math.min(d,(video.currentTime||0)+(e.key==='ArrowRight'?step:-step)))});
}
...
function youtubeSearch(){const q=$('#producerYoutubeSearchV4210')?.value?.trim();if(!q)return toast('Enter a YouTube search phrase.','Search needed');window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(q)}`,'_blank','noopener,noreferrer');toast('Search opened. Copy a video URL and paste it into the embedded preview.')}
