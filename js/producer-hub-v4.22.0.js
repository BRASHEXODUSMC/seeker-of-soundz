...
const duration=()=>Number(video?.duration||60)||60;
...
function skipRemovedRanges(){
 ...
 if(!hit)return;
 seekingCut=true;video.currentTime=Math.min(hit.end,duration());
 if(audio?.src)audio.currentTime=Math.min(video.currentTime,audio.duration||video.currentTime);
