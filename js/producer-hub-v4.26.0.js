...
function seekFromPointer(e,target){
 ...
 if($('#snapPlayheadToClipEdgesV4260')?.checked&&window.SOSVideoClipsV4240?.snapToClipEdge){
  const snapped=window.SOSVideoClipsV4240.snapToClipEdge(virtualTimelineTime,.16);
  if(Math.abs(snapped-virtualTimelineTime)<=.16)virtualTimelineTime=snapped;
 }
 positionPlayhead(virtualTimelineTime/Math.max(.01,d));
...
function updateButtons(){
 ...
 undoButton.disabled=!undoStack.length&&!window.SOSVideoClipsV4240?.canUndoEdit?.();
 redoButton.disabled=!redoStack.length&&!window.SOSVideoClipsV4240?.canRedoEdit?.();
}
...
function snappedCutTime(time){
 const enabled=$('#snapCutsToClipEdgesV4260')?.checked;
 const tolerance=Number($('#cutSnapToleranceV4260')?.value||.2);
 if(!enabled||!window.SOSVideoClipsV4240?.snapToClipEdge)return time;
 const snapped=window.SOSVideoClipsV4240.snapToClipEdge(time,tolerance);
 return Math.abs(snapped-time)<=tolerance?snapped:time;
}
function markStart(){
 cutStart=snappedCutTime(currentTime());if(cutEnd!==null&&cutEnd<=cutStart)cutEnd=null;
 status.textContent=`Cut start marked at ${format(cutStart)}. Move the playhead and mark the end.`;updateButtons();
}
function markEnd(){
 cutEnd=snappedCutTime(currentTime());
...
async function applyCut(){
 ...
 const start=cutStart,end=cutEnd;
 const ripple=$('#rippleDeleteCutsV4260')?.checked!==false;
 const snap=$('#snapCutsToClipEdgesV4260')?.checked!==false;
 const tolerance=Number($('#cutSnapToleranceV4260')?.value||.2);
 if(ripple&&window.SOSVideoClipsV4240?.rippleDelete){
  const result=await window.SOSVideoClipsV4240.rippleDelete(start,end,{snap,tolerance});
  cutStart=null;cutEnd=null;cuts=[];renderCuts();updateButtons();
  status.textContent=result.ok
   ?`Ripple cut removed ${format(result.removed)}. The following video now fills the gap.`
   :(result.message||'The ripple cut could not be applied.');
  return;
 }
 saveHistory();cuts=normalize([...cuts,{start,end}]);cutStart=null;cutEnd=null;renderCuts();updateButtons();
 status.textContent='Section removed non-destructively. Preview and export will skip it.';
}
function undo(){
 if(window.SOSVideoClipsV4240?.canUndoEdit?.()){
  window.SOSVideoClipsV4240.undoEdit();status.textContent='Ripple edit undone. Video segments and gaps were restored.';updateButtons();return;
 }
...
function redo(){
 if(window.SOSVideoClipsV4240?.canRedoEdit?.()){
  window.SOSVideoClipsV4240.redoEdit();status.textContent='Ripple edit redone. Following clips close the gap again.';updateButtons();return;
 }
...
window.addEventListener('sos:ripple-cut-applied',()=>updateButtons());
