...
function layerBox(layer){
 ...
 element.addEventListener('pointerdown',event=>{
  ...
  event.preventDefault();selectLayer(layer.id);
  const rect=stage.getBoundingClientRect();drag={id:layer.id,pointer:event.pointerId,rect};
  element.setPointerCapture?.(event.pointerId);
 });
 rotate.addEventListener('pointerdown',event=>{
  event.preventDefault();event.stopPropagation();selectLayer(layer.id);
...
stage.addEventListener('pointermove',event=>{
 if(drag){
  ...
  layer.x=clamp((event.clientX-drag.rect.left)/drag.rect.width,0,1);layer.y=clamp((event.clientY-drag.rect.top)/drag.rect.height,0,1);
  renderStage();renderEditor();redraw();
 }
 if(rotating){
  ...
  layer.rotation=Math.round(rotating.startRotation+angle-rotating.startAngle);renderStage();renderEditor();redraw();
 }
});
stage.addEventListener('pointerup',()=>{drag=null;rotating=null});
stage.addEventListener('pointercancel',()=>{drag=null;rotating=null});
