...
const offsetPreset=$('#musicOffsetPresetV4350'),customOffset=$('#musicCustomOffsetV4350'),customOffsetWrap=$('#musicCustomOffsetWrapV4350');
const startLabel=$('#musicRangeStartLabelV4350'),endLabel=$('#musicRangeEndLabelV4350');
const startHandle=$('#musicTrimStartV4330'),endHandle=$('#musicTrimEndV4330'),selection=$('#musicTrimSelectionV4330');

function initializeMusicRangeV701(force=false){
 const total=Number(audio?.duration||0);
 if(!(total>0))return false;
 const invalid=!(musicTrim.sourceEnd>musicTrim.sourceStart+.01)||musicTrim.sourceEnd>total+.05||musicTrim.duration!==total;
 musicTrim.duration=total;
 if(force||invalid){
  musicTrim.sourceStart=Math.max(0,Math.min(total-.01,Number(startInput?.value||0)));
  const requestedEnd=Number(endInput?.value||0);
  musicTrim.sourceEnd=requestedEnd>musicTrim.sourceStart+.01?Math.min(total,requestedEnd):total;
  musicTrim.projectOffset=Math.max(0,Number(offsetInput?.value||0));
 }
 audio.muted=false;
...
function normalizeMusic(){
 initializeMusicRangeV701(false);
 ...
 const requestedEnd=Number(endInput?.value||musicTrim.sourceEnd||0);
 musicTrim.sourceEnd=requestedEnd>musicTrim.sourceStart+.01
  ?Math.min(total,requestedEnd)
  :total;
 ...
 if(offsetPreset){
  const presetValues=['0','1','2','5','10'];
  const exact=String(Number(musicTrim.projectOffset));
  if(presetValues.includes(exact)){offsetPreset.value=exact;if(customOffsetWrap)customOffsetWrap.hidden=true}
  else{offsetPreset.value='custom';if(customOffsetWrap)customOffsetWrap.hidden=false;if(customOffset)customOffset.value=musicTrim.projectOffset.toFixed(2)}
 }
 if(startLabel)startLabel.textContent=`Start ${formatMusicTimeV4350(musicTrim.sourceStart)}`;
 if(endLabel)endLabel.textContent=`End ${formatMusicTimeV4350(musicTrim.sourceEnd)}`;
 renderMusicTrim();
...
offsetPreset?.addEventListener('change',()=>{
 const value=offsetPreset.value;
 const custom=value==='custom';
 if(customOffsetWrap)customOffsetWrap.hidden=!custom;
 if(custom){
  customOffset.value=String(musicTrim.projectOffset||0);
  customOffset.focus();
 }else{
  offsetInput.value=String(Number(value)||0);
  normalizeMusic();
 }
});
customOffset?.addEventListener('input',()=>{
 offsetInput.value=String(Math.max(0,Number(customOffset.value)||0));
 normalizeMusic();
});
[startInput,endInput,offsetInput].forEach(input=>input?.addEventListener('input',normalizeMusic));
function onMusicReadyV701(){
 if(!initializeMusicRangeV701(true))return;
 musicTrim.sourceStart=0;
 musicTrim.sourceEnd=audio.duration;
 musicTrim.projectOffset=Math.max(0,Number(offsetInput?.value||0));
 if(startInput)startInput.value='0';
 if(endInput)endInput.value=String(audio.duration);
 normalizeMusic();
}
['loadedmetadata','durationchange','canplay'].forEach(name=>audio?.addEventListener(name,onMusicReadyV701));
if(audio?.readyState>=1&&Number(audio.duration)>0)onMusicReadyV701();
$('#previewMusicRangeV4350')?.addEventListener('click',async()=>{
 if(!audio?.src)return;
 initializeMusicRangeV701(false);normalizeMusic();
 audio.muted=false;audio.defaultMuted=false;audio.volume=1;
 const context=window.SOSProducerAudioV701?.context?.();
 if(context?.state==='suspended')await context.resume().catch(()=>{});
 window.SOSVideoClipsV4240?.seekGlobal?.(musicTrim.projectOffset);
 audio.currentTime=musicTrim.sourceStart;
 try{await audio.play()}catch(error){console.warn('[Producer Hub 7.0.1] Music preview',error)}
 clearTimeout(musicAuditionTimer);
 musicAuditionTimer=setTimeout(()=>{
  if(audio.currentTime>=musicTrim.sourceEnd-.05||audio.paused)return;
  audio.pause();
 },Math.min(12000,Math.max(500,(musicTrim.sourceEnd-musicTrim.sourceStart)*1000)));
});
...
audio?.addEventListener('timeupdate',()=>{
 if(!(musicTrim.sourceEnd>musicTrim.sourceStart+.01))return;
 if(audio.currentTime>=musicTrim.sourceEnd-.025&&!audio.paused){
  audio.pause();
  audio.currentTime=Math.max(musicTrim.sourceStart,Math.min(musicTrim.sourceEnd-.01,audio.currentTime));
 }
});
