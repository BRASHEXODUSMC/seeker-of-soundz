...
async function addFiles(files,replace=false){
 ...
 for(const file of valid){
  ...
  try{
   ...
   clips.push({id:uid(),name:file.name,url,file,duration:meta.duration,muted:false,width:meta.width,height:meta.height,thumbnail:meta.thumbnail||'',transitionAfter:'none',transitionDuration:.5});
...
function adoptPrimaryFile(){
 ...
 setTimeout(async()=>{
  ...
  clips=[{id:uid(),name:file.name,url,file,duration:durationValue,muted:false,width:video.videoWidth,height:video.videoHeight,thumbnail:'',transitionAfter:'none',transitionDuration:.5}];
...
window.SOSVideoClipsV4240={totalDuration,globalTime,hasMultiple,hasNext,startSequence,seekGlobal,getClips:()=>clips.map(c=>({...c,file:undefined})),activeIndex:()=>activeIndex,offsetFor,clipsRef:()=>clips};
