...
async function trimSegment(id,edge,deltaSeconds,options={}){
 ...
 if(options.snap&&window.SOSProjectRangeV4300?.snapTime){
  const projectEdge=edge==='left'?offsetFor(index):offsetFor(index)+clip.duration;
  const snapped=window.SOSProjectRangeV4300.snapTime(projectEdge+delta);
  delta=snapped-projectEdge;
 }
 if(edge==='left'){
 ...
 await seekGlobal(Math.min(offsetFor(index),totalDuration()));
 window.dispatchEvent(new CustomEvent('sos:clip-trimmed',{detail:{id,edge,delta,duration:clip.duration}}));
