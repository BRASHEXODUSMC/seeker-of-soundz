...
async function loadIndex(index,localTime=0,autoplay=false){
 ...
 if(Math.abs((video.currentTime||0)-target)>.015){
  await new Promise(resolve=>{
...
function renderClips(){
 ...
 videoLane.innerHTML=clips.map((clip,index)=>{
  ...
  const boundary=index<clips.length-1?`<div class="razorBoundaryV4270" style="left:${offset/total*100}%" data-video-boundary="${index}">
    <button type="button" data-delete-video-left="${clip.id}" title="Delete video section on the left">×L</button>
    <i></i>
    <button type="button" data-delete-video-right="${clips[index+1].id}" title="Delete video section on the right">R×</button>
   </div>`:'';
  ...
  return `<section class="timelineVideoSubLaneV4250" data-video-lane="${index}" style="--lane-index:${index}">
   ...
   <button type="button" class="timelineVideoSegmentV4240 ${index===activeIndex?'isActive isSelectedV4310':''} ${clip.muted?'isMuted':''}" data-open-index="${index}" data-video-segment-id="${clip.id}" style="left:${left}%;width:${width}%;background-image:linear-gradient(90deg,rgba(42,18,62,.42),rgba(111,53,153,.28)),url('${clip.thumbnail||''}')"><span class="clipTrimHandleV4310 isLeft" data-trim-left="${clip.id}" title="Trim clip start"></span><strong>Video ${index+1}</strong><small>${clip.muted?'Muted · ':''}${fmt(clip.duration)}</small><span class="clipTrimHandleV4310 isRight" data-trim-right="${clip.id}" title="Trim clip end"></span></button>
   ${connector}${boundary}
  </section>`;
 }).join('');
 ...
 videoLane.querySelectorAll('[data-open-index]').forEach(button=>button.addEventListener('click',event=>{
  if(event.target.closest('[data-trim-left],[data-trim-right]'))return;
  event.stopPropagation();activeIndex=Number(button.dataset.openIndex);renderClips();loadIndex(activeIndex,0,false);
 }));
 ...
 videoLane.querySelectorAll('[data-delete-video-left],[data-delete-video-right]').forEach(button=>button.addEventListener('click',async event=>{
  event.preventDefault();event.stopPropagation();
  const id=button.dataset.deleteVideoLeft||button.dataset.deleteVideoRight;
...
async function trimSegment(id,edge,deltaSeconds,options={}){
 const index=clips.findIndex(clip=>clip.id===id);
 if(index<0)return {ok:false,message:'Clip not found.'};
 const clip=ensureTrim(clips[index]);
 const minDuration=Math.max(.08,Number(options.minDuration||.12));
 let delta=Number(deltaSeconds)||0;
 if(options.snap){
  const projectEdge=edge==='left'?offsetFor(index):offsetFor(index)+clip.duration;
  const proposed=projectEdge+delta;
  const tolerance=Math.max(.04,Number(options.tolerance||.16));
  const clipCandidates=clipEdges().filter(candidate=>Math.abs(candidate-projectEdge)>tolerance*.6);
  const beatCandidates=[...document.querySelectorAll('[data-marker-time]')]
   .map(marker=>Number(marker.dataset.markerTime)).filter(Number.isFinite);
  const candidates=[0,totalDuration(),...clipCandidates,...beatCandidates];
  const nearest=candidates.reduce((best,value)=>Math.abs(value-proposed)<Math.abs(best-proposed)?value:best,proposed);
  if(Math.abs(nearest-proposed)<=tolerance)delta=nearest-projectEdge;
 }
 if(edge==='left'){
  const next=Math.max(0,Math.min(clip.sourceEnd-minDuration,clip.sourceStart+delta));
  delta=next-clip.sourceStart;
  if(Math.abs(delta)<.001)return {ok:false,message:'No trim change.'};
  pushEditHistory();clip.sourceStart=next;
 }else{
  const next=Math.min(clip.originalDuration,Math.max(clip.sourceStart+minDuration,clip.sourceEnd+delta));
  delta=next-clip.sourceEnd;
  if(Math.abs(delta)<.001)return {ok:false,message:'No trim change.'};
  pushEditHistory();clip.sourceEnd=next;
 }
 ensureTrim(clip);clips[index]=clip;activeIndex=index;updateEnd();renderClips();
 await loadIndex(index,0,false);
 clipStatus.textContent=`${edge==='left'?'Start':'End'} trimmed. Video ${index+1} is now ${fmt(clip.duration)} long.`;
 window.dispatchEvent(new CustomEvent('sos:clip-trimmed',{detail:{id,edge,delta,duration:clip.duration,sourceStart:clip.sourceStart,sourceEnd:clip.sourceEnd}}));
 return {ok:true,clip:{...clip,file:undefined}};
}

async function splitAt(time,options={}){
 if(!clips.length)return {ok:false,message:'No video clips are loaded.'};
 const tolerance=Math.max(0,Number(options.tolerance??.12));
 time=maybeSnap(Math.max(0,Math.min(totalDuration(),Number(time)||0)),options.snap===true,tolerance);
 const location=clipAtTime(time),index=location.index,clip=ensureTrim(clips[index]);
 const sourcePoint=clip.sourceStart+location.local;
 if(sourcePoint<=clip.sourceStart+.025||sourcePoint>=clip.sourceEnd-.025){
  return {ok:false,message:'Move the razor inside a video section before splitting.'};
 }
 pushEditHistory();
 const left=ensureTrim({...clip,id:uid(),sourceEnd:sourcePoint,transitionAfter:'none'});
 const right=ensureTrim({...clip,id:uid(),sourceStart:sourcePoint,transitionAfter:clip.transitionAfter});
 clips.splice(index,1,left,right);
 activeIndex=index;
 updateEnd();renderClips();await seekGlobal(time);
 window.dispatchEvent(new CustomEvent('sos:video-split',{detail:{time,leftId:left.id,rightId:right.id}}));
 return {ok:true,time,left,right};
}
async function deleteSegment(id){
 ...
 if(removed.url?.startsWith('blob:')&&!clips.some(clip=>clip.url===removed.url))URL.revokeObjectURL(removed.url);
 activeIndex=Math.min(index,Math.max(0,clips.length-1));
 updateEnd();renderClips();
 if(clips.length)await loadIndex(activeIndex,0,false);
 else{video.pause();video.removeAttribute('src');video.load()}
 clipStatus.textContent=`Video section deleted. Remaining clips ripple together. Project length: ${fmt(totalDuration())}.`;
 window.dispatchEvent(new CustomEvent('sos:video-segment-deleted',{detail:{id,totalDuration:totalDuration()}}));
 return true;
}
...
window.SOSVideoClipsV4240={
 ...
 offsetFor,clipsRef:()=>clips,clipEdges,snapToClipEdge,rippleDelete,splitAt,deleteSegment,trimSegment,
 rerender:renderClips,selectClip:index=>{activeIndex=Math.max(0,Math.min(clips.length-1,Number(index)||0));renderClips();},
 hasClips:()=>clips.length>0,currentClip:()=>clips[activeIndex]?({...clips[activeIndex],file:undefined}):null,
...
function renderEffectRegions(){
 ...
 effectLane.innerHTML=effectRegions.map((region,index)=>`<article class="timedEffectRegionV4240 ${region.muted?'isMuted':''}" data-effect-region-index="${index}" draggable="true" style="left:${region.start/total*100}%;width:${(region.end-region.start)/total*100}%">
...
window.SOSTimedEffectsV4240={
 ...
 addRegions:regions=>{
  ...
  effectRegions.push(...regions.map(region=>({...region,muted:false})));
  effectRegions.sort((a,b)=>a.start-b.start);
  renderEffectRegions();
 },
 replaceAutoRegions:regions=>{
  effectRegions=effectRegions.filter(region=>!region.auto);
  if(Array.isArray(regions)&&regions.length)effectRegions.push(...regions.map(region=>({...region,muted:false,auto:true})));
  effectRegions.sort((a,b)=>a.start-b.start);
  renderEffectRegions();
 },
 moveRegion:(index,start)=>{
  const region=effectRegions[Number(index)];if(!region)return false;
  const length=Math.max(.01,region.end-region.start),total=Math.max(.01,totalDuration());
  region.start=Math.max(0,Math.min(total-length,Number(start)||0));region.end=region.start+length;
  effectRegions.sort((a,b)=>a.start-b.start);renderEffectRegions();return true;
 },
 resizeRegion:(index,start,end)=>{
  const region=effectRegions[Number(index)];if(!region)return false;
  region.start=Math.max(0,Number(start)||0);region.end=Math.max(region.start+.01,Number(end)||region.end);
  effectRegions.sort((a,b)=>a.start-b.start);renderEffectRegions();return true;
 },
 render:renderEffectRegions,
