...
$('#gpuTimelineToggleV4320')?.addEventListener('click',event=>{event.currentTarget.classList.toggle('isActive');event.currentTarget.textContent=event.currentTarget.classList.contains('isActive')?'GPU Cache ON':'GPU Cache OFF';updateGpu()});
updateGpu();
