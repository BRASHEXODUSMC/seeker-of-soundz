...
async function downloadObject(object,name){
...
const yieldToBrowser=()=>new Promise(resolve=>(window.requestIdleCallback||window.setTimeout)(resolve,{timeout:250}));
function portableMediaLimit(){
 const memory=Number(navigator.deviceMemory||4);
 if(memory<=2)return 32*1024*1024;
 if(memory<=4)return 64*1024*1024;
 return 96*1024*1024;
}
async function buildProject(embed){
 ...
 const project={
  ...
  mediaSettings:{
   video:window.SOSMediaSettingsV4330?.video?.()||null,
   music:window.SOSMediaSettingsV4330?.music?.()||null
  },
 ...
 if(embed){
  ...
  const mediaFiles=[...unique.values(),...(audioInput?.files?.[0]?[audioInput.files[0]]:[])];
  const total=mediaFiles.reduce((sum,file)=>sum+Number(file.size||0),0);
  const largest=mediaFiles.reduce((max,file)=>Math.max(max,Number(file.size||0)),0);
  const limit=portableMediaLimit();
  const estimatedJson=Math.ceil(total*1.38);
  if(largest>64*1024*1024||total>limit||estimatedJson>128*1024*1024){
   project.media.embedded=false;
   project.media.embeddingSkipped=true;
   project.media.embeddingReason=`Media was not embedded to protect this browser. Selected media: ${Math.round(total/1024/1024)} MB; safe limit: ${Math.round(limit/1024/1024)} MB.`;
  }else{
   for(const file of unique.values()){
    await yieldToBrowser();
    project.media.video.push({name:file.name,type:file.type,size:file.size,data:await fileToDataURL(file)});
   }
   if(audioInput?.files?.[0]){
    await yieldToBrowser();
    const file=audioInput.files[0];
    project.media.audio={name:file.name,type:file.type,size:file.size,data:await fileToDataURL(file)};
   }
  }
 }
 return project;
...
$('#saveProjectV4320')?.addEventListener('click',async()=>{
 const status=$('#projectStatusV4320');try{
  ...
  const size=await downloadObject(project,`${project.name.replace(/[^\w-]+/g,'-')||'project'}.sosproject`);
  status.textContent=project.media.embedded
   ?`Portable project downloaded (${Math.max(1,Math.round(size/1024/1024))} MB).`
   :project.media.embeddingSkipped
    ?`${project.media.embeddingReason} A safe settings-only project was downloaded instead.`
    :'Project downloaded safely. Re-link original media after import.';
 }catch(error){status.textContent=error.message}
});
$('#savePresetV4320')?.addEventListener('click',async()=>{
 const status=$('#projectStatusV4320');
 try{
  await downloadObject({kind:'seeker-of-soundz-preset',version:state.projectVersion,controls:serializeControls(),pro:{keyframes:state.keyframes,directorRegions:state.directorRegions,curve:state.curve}},'seeker-of-soundz-settings.sospreset');
  if(status)status.textContent='Settings preset downloaded safely to this computer.';
 }catch(error){if(status)status.textContent=`Preset save failed: ${error.message}`}
});
async function importObject(file,isProject){
 if(!file)throw new Error('Choose a project or preset file first.');
 const safeImportLimit=160*1024*1024;
 if(file.size>safeImportLimit)throw new Error(`This project file is ${Math.round(file.size/1024/1024)} MB, which is too large to import safely in this browser. Use a settings-only project and re-link the original media.`);
 await yieldToBrowser();
 ...
 if(data.mediaSettings?.video?.mode)window.SOSMediaSettingsV4330?.applyVideoSizing?.(data.mediaSettings.video.mode);
 if(data.mediaSettings?.music){
  const music=data.mediaSettings.music;
  const start=document.getElementById('musicSourceStartV4330'),end=document.getElementById('musicSourceEndV4330'),offset=document.getElementById('musicProjectOffsetV4330');
  if(start)start.value=music.sourceStart??0;if(end)end.value=music.sourceEnd??0;if(offset)offset.value=music.projectOffset??0;
