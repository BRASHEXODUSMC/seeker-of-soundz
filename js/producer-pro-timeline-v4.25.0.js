...
/* Smooth inertial horizontal scrolling */
viewport.addEventListener('wheel',event=>{
 if(Math.abs(event.deltaX)<1&&Math.abs(event.deltaY)<1)return;
 if(event.shiftKey||zoom>1||Math.abs(event.deltaX)>Math.abs(event.deltaY)){
  event.preventDefault();
  inertialVelocity+=(Math.abs(event.deltaX)>Math.abs(event.deltaY)?event.deltaX:event.deltaY)*.7;
  cancelAnimationFrame(inertialFrame);
  const glide=()=>{
   viewport.scrollLeft+=inertialVelocity;
   inertialVelocity*=.88;
   if(Math.abs(inertialVelocity)>.35)inertialFrame=requestAnimationFrame(glide);
  };
...
const visualPatterns={
 balanced:['beatZoom','dropFlash','glow','rgb'],
 heavy:['dropFlash','shake','beatZoom','strobe'],
 cinematic:['lightLeaks','horizonGlow','beatZoom','glow'],
 chaos:['glitchBars','rgb','shake','dropFlash']
};
function beatRows(){
...
function generateAutoSync(){
 ...
 const visuals=visualPatterns[pattern]||visualPatterns.balanced;
 ...
 beats.forEach((beat,index)=>{
  ...
  const length=Math.max(.12,Math.min(1.2,(next-beat.time)*.9));
  if($('#autoSyncAddCameraV4250')?.checked){
   cameraRegions.push({start:beat.time,end:Math.min(duration(),beat.time+length),motion:motions[(index/density|0)%motions.length],strength:beat.drop?strength*1.35:strength,auto:true});
  }
  if($('#autoSyncAddVisualsV4250')?.checked){
   const effect=visuals[(index/density|0)%visuals.length];
   const input=$(`[data-effect="${CSS.escape(effect)}"]`);
   const name=input?.closest('label')?.querySelector('strong')?.textContent?.trim()||effect;
   const region={effect,name,start:beat.time,end:Math.min(duration(),beat.time+length),auto:true,beat:true};
   autosyncEffectRegions.push(region);window.SOSTimedEffectsV4240?.addRegion?.(region);
  }
 });
 renderCameraRegions();
 $('#autoSyncStatusV4250').textContent=`Generated ${cameraRegions.length} true camera moves and ${autosyncEffectRegions.length} timed visual regions from ${beats.length} beat markers.`;
 return true;
}
