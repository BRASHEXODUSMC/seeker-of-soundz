...
const cameraPatterns={
 ...
 'bass-distortion':['auto-bass-warp','auto-impact','auto-bass-crush','auto-shake','auto-bass-lens'],
 'bass-warp':['auto-bass-warp','auto-bass-twist','auto-bass-crush','auto-zoom-in'],
 'drop-impact':['auto-bass-lens','auto-impact','auto-shake','auto-zoom-out','auto-bass-crush']
};
...
function generateAutoSyncNow(){
 ...
 cameraRegions=[];autosyncEffectRegions=[];
 beats.forEach((beat,index)=>{
  ...
  if($('#autoSyncAddCameraV4250')?.checked){
   const motion=motions[(index/density|0)%motions.length];
   const distortionEnabled=$('#autoSyncBassDistortionV4300')?.checked!==false&&beat.drop;
   const distortionMode=$('#autoSyncDropDistortionV4300')?.value||'mixed';
   cameraRegions.push({
    start:beat.time,end:Math.min(duration(),beat.time+baseLength),motion,
    strength:beat.drop?strength*1.35:strength,auto:true,drop:beat.drop,
    distortion:distortionEnabled?(distortionMode==='mixed'?['warp','crush','lens','twist'][index%4]:distortionMode):'none'
   });
  }
  if($('#autoSyncAddVisualsV4250')?.checked&&visuals.length){
   for(let layer=0;layer<overlap;layer++){
    ...
    autosyncEffectRegions.push(region);
   }
  }
 });
 window.SOSTimedEffectsV4240?.replaceAutoRegions?.(autosyncEffectRegions);
 renderCameraRegions();
...
function generateAutoSync(){
 const panel=$('.autoSyncMotionPanelV4250');
 const status=$('#autoSyncStatusV4250');
 if(panel?.classList.contains('isGeneratingV4290'))return false;
 panel?.classList.add('isGeneratingV4290');
 if(status)status.textContent='Preparing beat, camera, particle, and overlay regions…';
 requestAnimationFrame(()=>requestAnimationFrame(()=>{
  try{generateAutoSyncNow()}
  finally{panel?.classList.remove('isGeneratingV4290')}
 }));
 return true;
}
