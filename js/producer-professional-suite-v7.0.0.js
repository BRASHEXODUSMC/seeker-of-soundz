...
function renderTracks(){
 ...
 host.querySelectorAll('[data-track-action]').forEach(btn=>btn.onclick=()=>{
  const t=state.tracks[btn.dataset.track],a=btn.dataset.trackAction;t[a==='lock'?'locked':a]=!t[a==='lock'?'locked':a];
...
function applyTrackState(){
 ...
 Object.entries(map).forEach(([name,sel])=>{const el=$(sel),t=state.tracks[name];if(!el)return;el.classList.toggle('studioTrackLockedV700',t.locked);el.classList.toggle('studioTrackMutedV700',t.muted||(anySolo&&!t.solo));el.style.setProperty('--studio-track-color',t.color)});
}
renderTracks();
$('#colorSelectedClipV700')?.addEventListener('click',()=>{state.tracks.video.color=$('#clipColorV700').value;renderTracks();applyTrackState()});
...
window.SOSProfessionalSuiteV700={
 ...
 musicGain(){const a=state.audio;const eq=Math.pow(10,(a.bass+a.mid+a.treble)/60);return Math.min(a.limiter,eq*(1-a.compressor*.12))},
