...
videoLane.addEventListener('pointerdown',event=>{
 ...
 const clipState=(window.SOSVideoClipsV4240?.clipsRef?.()||[]).find(clip=>clip.id===(handle.dataset.trimLeft||handle.dataset.trimRight));
 trimDrag={
  ...
  sourceStart:Number(clipState?.sourceStart||0),sourceEnd:Number(clipState?.sourceEnd||0),
  originalDuration:Number(clipState?.originalDuration||clipState?.duration||0),
  wasPlaying:!document.querySelector('#composerPreviewVideoV4180')?.paused
 };
...
window.addEventListener('pointermove',event=>{
 ...
 const previewVideo=document.querySelector('#composerPreviewVideoV4180');
 if(previewVideo){
  const min=.08;
  const sourcePoint=trimDrag.edge==='left'
   ?Math.max(0,Math.min(trimDrag.sourceEnd-min,trimDrag.sourceStart+trimDrag.delta))
   :Math.max(trimDrag.sourceStart,Math.min(trimDrag.originalDuration,trimDrag.sourceEnd+trimDrag.delta)-min);
  if(Number.isFinite(sourcePoint)&&Math.abs((previewVideo.currentTime||0)-sourcePoint)>.025)previewVideo.currentTime=sourcePoint;
 }
 const seconds=Math.abs(trimDrag.delta||0).toFixed(2);
 status.textContent=`${trimDrag.edge==='left'?'Start':'End'} trim preview: ${trimDrag.delta>=0?'+':'−'}${seconds}s · release to use this frame`;
},true);
window.addEventListener('pointerup',async event=>{
 ...
 status.textContent=result?.ok
  ?`Clip ${current.edge==='left'?'start':'end'} trimmed successfully. Playback now begins inside the selected handles.`
  :(result?.message||'The clip could not be trimmed there.');
 if(result?.ok&&current.wasPlaying){
  document.querySelector('#composerPreviewVideoV4180')?.play().catch(()=>{});
 }
},true);
