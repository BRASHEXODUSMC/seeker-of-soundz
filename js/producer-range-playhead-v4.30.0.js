...
function timeFromPointer(event){
 ...
 return snapTime(Math.max(0,Math.min(1,(event.clientX-rect.left)/Math.max(1,rect.width)))*duration());
}
function begin(event,type){
 event.preventDefault();event.stopPropagation();
 dragging={type,pointer:event.pointerId};
 event.currentTarget.setPointerCapture?.(event.pointerId);
 document.body.classList.add('isDraggingTimelineRangeV4300');
}
startHandle.addEventListener('pointerdown',event=>begin(event,'start'));
endHandle.addEventListener('pointerdown',event=>begin(event,'end'));
window.addEventListener('pointermove',event=>{
 if(!dragging)return;
 const time=timeFromPointer(event);
 const {start,end}=normalizeRange();
 if(dragging.type==='start')startInput.value=Math.min(time,end-.01).toFixed(2);
 else endInput.value=Math.max(time,start+.01).toFixed(2);
 renderRange();
...
function finish(){
 if(!dragging)return;
 dragging=null;document.body.classList.remove('isDraggingTimelineRangeV4300');
