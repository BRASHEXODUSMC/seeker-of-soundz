...
function render(){
 host.hidden=false;if(clear)clear.hidden=false;
 document.querySelector('.renderQueueV4190')?.classList.toggle('isEmptyV701',!rows.length);
...
clear?.addEventListener('click',()=>{
 rows=rows.filter(row=>!['complete','completed','finished','failed','cancelled','canceled','done'].includes(String(row.status).toLowerCase()));
 localStorage.setItem(STORAGE,JSON.stringify(rows.slice(0,30)));
 render();
 if(!rows.length){
  host.innerHTML='';
  host.hidden=true;
  clear.hidden=true;
  document.querySelector('.renderQueueV4190')?.classList.add('isEmptyV701');
 }
});
render();
})();