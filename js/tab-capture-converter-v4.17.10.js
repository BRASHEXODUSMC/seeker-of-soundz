...
async function startCaptureFlow(options={}){
 pendingAutoRecord=!!options.autoRecord;downloadVideo.disabled=true;downloadWav.disabled=true;downloadWave.disabled=true;recordedBlob=null;wavBlob=null;
...
function startRecording(){
 ...
 recorder.onstop=()=>{
  ...
  if(sampleCount>Math.max(4096,(audioContext?.sampleRate||44100)/4)&&audioContext){
   ...
   setStatus('isSupported','Recording complete','Recorded video, real captured-audio WAV, and waveform PNG are ready.');
  }else{
   wavBlob=null;downloadWav.disabled=true;
   setStatus('isLimited','Video recorded, but no usable audio samples were captured','Repeat capture and enable Share tab audio in the browser picker to create the WAV.');
  }
