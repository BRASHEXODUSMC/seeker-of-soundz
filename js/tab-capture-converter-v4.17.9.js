...
let animation=0,recorder=null,chunks=[],recordedBlob=null,wavBlob=null,pcmChunks=[],recording=false,lastTime=0,pendingAutoRecord=false;
...
function effects(ctx,w,h,levels){
 ...
 if(fx.includes('letterbox')){const b=h*.1;ctx.fillStyle='#000';ctx.fillRect(0,0,w,b);ctx.fillRect(0,h-b,w,b)}
...
async function startCaptureFlow(options={}){
 pendingAutoRecord=!!options.autoRecord;
 ...
 try{
  ...
  if(audioTracks.length){
   ...
   processor=audioContext.createScriptProcessor(4096,1,1);silentGain=audioContext.createGain();silentGain.gain.value=0;
   ...
   processor.onaudioprocess=e=>{if(recording){const input=e.inputBuffer.getChannelData(0);pcmChunks.push(new Float32Array(input))}};
  ...
  if(pendingAutoRecord){
   setStatus(audioTracks.length?'isSupported':'isLimited','Capture approved — recording starts automatically',audioTracks.length?'The selected tab video and audio are now recording with the chosen effects.':'The selected tab is recording without audio. Restart and enable Share tab audio for WAV output.');
   setTimeout(()=>{if(displayStream&&!recording)startRecording()},450);
  }
...
function startRecording(){
 ...
 recorder.onstop=()=>{
  ...
  if(pcmChunks.length&&audioContext){wavBlob=makeWav(pcmChunks,audioContext.sampleRate);downloadWav.disabled=false}
  downloadWave.disabled=false;setStatus('isSupported','Recording complete','Video, captured audio WAV, and real waveform downloads are ready.');
 };
 recording=true;recorder.start(250);startRecord.disabled=true;stopRecord.disabled=false;setStatus('isSupported','Recording with selected effects','Keep the shared video playing until you press Stop.');
}
...
startCapture.onclick=()=>startCaptureFlow({autoRecord:false});startRecord.onclick=startRecording;stopRecord.onclick=stopRecording;
...
window.addEventListener('sos:start-youtube-tab-capture',event=>{
 ...
 setStatus('isWaiting','Choose the YouTube playback tab','In the browser share dialog, choose the newly opened YouTube tab and enable Share tab audio.');
