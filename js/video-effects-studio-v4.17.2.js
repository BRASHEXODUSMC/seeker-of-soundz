...
const assetStatus=$('#videoAssetStatusV4172');
let sourceUrl='',sourceFile=null,sourceBlob=null,audioBuffer=null;
...
function prepareSource({url,file=null,blob=null,label='video'}){
 revokeLocal();sourceFile=file;sourceBlob=blob||file||null;sourceUrl=url;
...
async function loadUrl(){
 ...
 prepareSource({url,label:'URL loaded'});
 try{
  ...
  waveformFailure(new Error(`The source server blocked audio analysis (${error.message}). The video can still play and its URL can still be downloaded.`));
 }
}
...
$('#loadVideoUrlV4171').onclick=loadUrl;$('#analyzeVideoV4171').onclick=()=>sourceBlob||sourceFile?analyzeFile(sourceBlob||sourceFile):loadUrl();
...
setReady('video',false);setReady('waveform',false);updateCount();bootAccess();
