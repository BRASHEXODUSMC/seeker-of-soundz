...
function showYouTube(url,id){
 sourceMode='youtube';youtubeUrl=url;sourceUrl=url;sourceFile=null;sourceBlob=null;
 video.pause();video.removeAttribute('src');video.load();
 if(nativeWrap)nativeWrap.hidden=true;
 if(youtubePreview)youtubePreview.hidden=false;
 if(youtubeFrame)youtubeFrame.src=`https://www.youtube.com/embed/${encodeURIComponent(id)}?rel=0`;
 if(openYouTube)openYouTube.href=url;
 setReady('video',true,'YouTube preview');
 setReady('waveform',false,'YouTube restricted');
 sourceDownload.disabled=true;
 waveDownload.disabled=true;
 drawWaveformPlaceholder('YouTube protects its media stream. Use a local file or direct MP4/WEBM URL for waveform editing.');
 setCompatibility('isPreview','YouTube preview works','Embedded playback is supported. Editing, waveform generation, effects rendering, and source download are unavailable for protected YouTube streams.');
 setStatus('YouTube preview loaded. Use a local file or direct MP4/WEBM URL to create waveforms and rendered effects.');
}
...
async function analyzeFile(blob){
 ...
 drawWaveform(audioBuffer);setReady('waveform',true,'download ready');
...
function previewLoop(){
 ...
 function draw(now){
  ...
  const effects=selectedEffects(),levels=energy();applyVideoCss(levels,effects);context.clearRect(0,0,width,height);drawEffects(context,width,height,levels,effects,false);
...
async function render(){
 if(sourceMode==='youtube')return setStatus('YouTube videos are preview-only. Upload the original video file or use a direct MP4/WEBM URL to render effects.');
...
function downloadWaveform(){
 ...
 const anchor=document.createElement('a');anchor.href=wave.toDataURL('image/png');anchor.download=`video-waveform-${Date.now()}.png`;anchor.click()
}
...
$('#loadVideoUrlV4171').onclick=loadUrl;$('#analyzeVideoV4171').onclick=()=>{
 if(sourceMode==='youtube')return setStatus('YouTube does not expose audio data for waveform generation. Use a local file or direct MP4/WEBM URL.');
 return sourceBlob||sourceFile?analyzeFile(sourceBlob||sourceFile):loadUrl()
};
waveDownload.onclick=downloadWaveform;sourceDownload.onclick=downloadSource;$('#renderVideoV4171').onclick=render;$('#exportProjectV4171').onclick=exportProject;
...
setReady('video',false);setReady('waveform',false);setCompatibility('isWaiting','Waiting for a video','Direct MP4/WEBM files support full editing. YouTube links support embedded preview only.');updateCount();bootAccess();
...
window.addEventListener('pagehide',()=>{cancelAnimationFrame(previewRaf);revokeLocal();audioContext?.close()},{once:true});
})();