...
function youtubePreviewLoop(){
 ...
 const loop=()=>{
  ...
  fillSyntheticArrays(time);applyYouTubeCss(levels,effects);context.clearRect(0,0,width,height);drawEffects(context,width,height,levels,effects,false);
 ...
 youtubePlayer=new YT.Player(target,{
  ...
  events:{
   onReady:event=>{
    ...
    drawSyntheticWaveform(youtubeDuration,0);waveDownload.disabled=false;setReady('waveform',true,'synchronized visual guide');
...
function showYouTube(url,id){
 ...
 createYouTubePlayer(id).catch(error=>{invalidUrl(error.message);});
}
