...
document.addEventListener('click',event=>{
 ...
 setTimeout(restore,40);setTimeout(restore,140);setTimeout(restore,320);
 if(control.matches('button'))setTimeout(()=>control.blur(),0);
},true);
document.addEventListener('click',event=>{
 const preset=event.target.closest('[data-effect-preset]');
 if(preset){event.preventDefault();event.stopPropagation();applyPreset(preset.dataset.effectPreset);setStatus(`${preset.textContent.trim()} preset applied with ${selectedEffects().length} effects.`);return}
