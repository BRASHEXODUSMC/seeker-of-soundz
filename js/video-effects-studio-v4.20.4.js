...
const converterSource=$('#converterSourceV4177'),converterWav=$('#converterWavV4177'),converterVideo=$('#converterVideoV4177'),converterStatus=$('#converterStatusV4177');
const stickyPreview=$('#stickyVideoPreviewV4177');
...
const clearYouTubeProjects=$('#clearYouTubeProjectsV4179'),playSelectedYouTube=$('#playSelectedYouTubeV4179'),captureSelectedYouTube=$('#captureSelectedYouTubeV4179');
const selectedCaptureThumb=$('#selectedCaptureThumbV41710'),selectedCaptureTitle=$('#selectedCaptureTitleV41710'),selectedCaptureUrl=$('#selectedCaptureUrlV41710');
...
const presets={
 ...
 animeEnergy:['beatZoom','chromaticPulse','animeBars','hueShift','sparkles','dropFlash','neonEdges','shake'],
 animeOpening:['lightLeaks','particles','animeBars','horizonGlow','hueShift','beatZoom','circleSpectrum'],
 jungle16:['pixelate','posterize','scanlines','particles','bounce','horizonGlow','vignette'],
 kart16:['pixelate','scanlines','laserGrid','shake','beatZoom','gridFlash','hueShift'],
 mode7:['tunnel','laserGrid','rotation','beatZoom','scanlines','pixelate','horizonGlow'],
 lowPolyRacer:['posterize','pixelate','vignette','retroCinemaBars','hueShift','shake','gridFlash'],
 arcadeDrift:['vhs','scanlines','rgb','channelGhost','horizonGlow','laserGrid','glow'],
 retroBoss:['heavyStatic','signalTear','dropFlash','shake','pixelate','chromaticPulse','retroCinemaBars'],
 broadcastStatic:['heavyStatic','scanlines','channelGhost','vignette'],
 damagedTape:['vhs','heavyStatic','signalTear','channelGhost','filmGrain','retroCinemaBars'],
 signalLoss:['heavyStatic','signalTear','rgb','dropFlash','scanlines','glitchBars'],
 crtArcade:['tvStatic','scanlines','channelGhost','pixelate','posterize','vignette','gridFlash']
};

function session(){return window.SOS?.getSession?.()||null}
function profileFrom(value){return value?.profile||value?.member||value?.user?.profile||value||null}
function roleLabel(value){
 const profile=profileFrom(value);
 return String(profile?.rankName||profile?.rank_name||profile?.role||value?.role||'member').toLowerCase().replace(/\s+/g,'_')
}
function allowed(){
 const current=session(),profile=profileFrom(current),role=roleLabel(current);
 const email=String(current?.user?.email||current?.email||profile?.email||'').toLowerCase();
 const collaboration=Boolean(profile?.collaborationAccess||profile?.collaboration_access||current?.collaborationAccess||current?.collaboration_access);
 return !!current&&(premiumRoles.has(role)||(collaboration&&role==='developer')||email==='brashexodus@gmail.com'||email==='romerojoseph95@gmail.com')
}
function setStatus(text){if(status)status.textContent=text}
function updateConverterState(mode='waiting',title='Waiting for an editable source',copy='Upload the original file or load a direct MP4/WEBM URL.'){
 if(!converterStatus)return;
 const dot=converterStatus.querySelector('[data-converter-dot]');
 const heading=converterStatus.querySelector('[data-converter-title]');
 const text=converterStatus.querySelector('[data-converter-copy]');
 if(dot)dot.className=`compatibilityDotV4173 ${mode}`;
 if(heading)heading.textContent=title;
 if(text)text.textContent=copy;
 const editable=sourceMode==='native'&&!!(sourceFile||sourceBlob||sourceUrl);
 if(converterSource)converterSource.disabled=!editable;
 if(converterWav)converterWav.disabled=!editable||!audioBuffer;
...
function updateLiveWaveformStatus(mode='waiting'){
 const host=$('#liveWaveformStatusV4176');if(!host)return;
 host.classList.toggle('isRecording',mode==='recording'||mode==='playing');
 const label=host.querySelector('small');
 if(label)label.textContent=mode==='recording'?'Recording video, audio and waveform history…':mode==='playing'?'Capturing visible waveform history while the video plays…':'Play the video to record waveform history. Local files also enable exact WAV and final video export.';
}
function recordWavePoint(time,levels,mode){
 ...
 updateLiveWaveformStatus(mode==='exact-audio'?'playing':'playing');playedWaveformHistory.push({time:Number(time.toFixed(3)),bass:Number(levels.bass.toFixed(4)),mid:Number(levels.mid.toFixed(4)),high:Number(levels.high.toFixed(4)),peak:Number(levels.peak.toFixed(4)),mode});
...
function renderRecentYouTube(){
 ...
 recentProjects.innerHTML=rows.length?rows.map(row=>`<article><img src="https://i.ytimg.com/vi/${row.id}/hqdefault.jpg" alt="YouTube thumbnail" loading="lazy"><div><strong>YouTube project</strong><small>${row.id}</small><div class="recentYouTubeActionsV4179"><button type="button" class="smallAction" data-use-recent-youtube="${row.url}">Use This Video</button><button type="button" class="smallAction dangerAction" data-delete-recent-youtube="${row.id}">Delete</button></div></div></article>`).join(''):'<p>No recent YouTube projects yet.</p>';
}
...
function youtubePreviewLoop(){
 ...
 const loop=()=>{
  ...
  const time=Number(youtubePlayer.getCurrentTime?.()||0),levels=youtubeLevels(time||performance.now()*.001),effects=selectedEffects();
 ...
 youtubePlayer=new YT.Player(target,{
  ...
  events:{
   onReady:event=>{
    ...
    try{event.target.mute();event.target.playVideo()}catch{}
...
function playCurrentYouTube(){
 if(!youtubePlayer)return setStatus('Load a YouTube project first.');
 try{
  youtubePlayer.unMute();
  youtubePlayer.playVideo();
  setStatus('YouTube video is playing with sound. Browser autoplay rules require this user click before sound can begin.');
 }catch(error){setStatus(error.message||'The YouTube player could not start.')}
}
function deleteRecentYouTube(id){
 let rows=[];try{rows=JSON.parse(localStorage.getItem('sos_recent_youtube_projects_v4175')||'[]')}catch{}
 rows=rows.filter(row=>row.id!==id);
 localStorage.setItem('sos_recent_youtube_projects_v4175',JSON.stringify(rows));renderRecentYouTube();
 setStatus('Saved YouTube project deleted.');
}
function clearRecentYouTube(){
 localStorage.removeItem('sos_recent_youtube_projects_v4175');renderRecentYouTube();setStatus('All saved YouTube projects cleared.');
}
function updateSelectedCaptureProject(url,id){
 if(selectedCaptureThumb)selectedCaptureThumb.src=`https://i.ytimg.com/vi/${id}/hqdefault.jpg`;
 if(selectedCaptureTitle)selectedCaptureTitle.textContent='YouTube project ready for tab capture';
 if(selectedCaptureUrl)selectedCaptureUrl.textContent=url;
 window.dispatchEvent(new CustomEvent('sos:youtube-project-selected',{detail:{url,id,thumbnail:`https://i.ytimg.com/vi/${id}/hqdefault.jpg`}}));
}
function showYouTube(url,id){
 ...
 updateConverterState('isInvalid','YouTube converter unavailable','Use the original video or audio file for conversion. Embedded YouTube media cannot be extracted by this page.');
 setStatus('Loading the YouTube player and synchronized effects preview…');
 saveRecentYouTube(url);updateSelectedCaptureProject(url,id);playedWaveformHistory=[];createYouTubePlayer(id).catch(error=>{invalidUrl(error.message);});
}
function invalidUrl(message='This URL is not a supported direct video or YouTube link.'){
 ...
 setCompatibility('isInvalid','Video not supported',message);updateConverterState('isInvalid','Converter unavailable',message);setStatus(message);
}
...
function bootAccess(){
 ...
 gate.hidden=false;
 studio.hidden=true;
 gate.innerHTML='<div><p class="sectionEyebrow">Premium Members Only</p><h2>Video Effects Studio is locked</h2><p>Sign in with an Owner, Administrator, Developer, or Premium Member account to use Producer Hub.</p><a class="primaryButton" href="members.html">Open Member Login</a></div>';
 return false;
}
...
function prepareSource({url,file=null,blob=null,label='video'}){
 ...
 updateConverterState('isSupported','Converter ready','This uploaded or direct media source can be converted to WAV and rendered video.');
...
async function analyzeFile(blob){
 ...
 updateConverterState('isSupported','Converter ready','The full decoded audio can now be exported as WAV, and the video can be rendered with selected effects.');
...
function drawStatic(context,width,height,amount){
 const density=Math.max(.05,Math.min(1,Number(amount)||.35));
 const count=Math.max(450,Math.round(width*height*.0016*density));
 context.save();
 context.globalCompositeOperation='screen';
 context.globalAlpha=.16+.34*density;
 for(let i=0;i<count;i++){
  const shade=Math.random()>.5?255:Math.floor(Math.random()*115);
  const alpha=.18+Math.random()*.72;
  context.fillStyle=`rgba(${shade},${shade},${shade},${alpha})`;
  const size=Math.random()<.88?1:2+Math.random()*3;
  context.fillRect(Math.random()*width,Math.random()*height,size,size);
 }
 // Animated rolling static bands make the effect visibly move even while paused.
 const bandY=(performance.now()*.22)%Math.max(1,height);
 const gradient=context.createLinearGradient(0,bandY-34,0,bandY+34);
 gradient.addColorStop(0,'rgba(255,255,255,0)');
 gradient.addColorStop(.5,`rgba(255,255,255,${.08+density*.2})`);
 gradient.addColorStop(1,'rgba(255,255,255,0)');
 context.fillStyle=gradient;
 context.fillRect(0,bandY-34,width,68);
 context.restore();
}
...
function drawEffects(context,width,height,levels,effects,rendering=false){
 const power=strength(),alpha=opacity(),{bass,mid,high,peak}=levels,time=sourceMode==='youtube'?Number(youtubePlayer?.getCurrentTime?.()||0):Number(video.currentTime||0);
 context.save();context.globalAlpha=alpha;
 if(effects.includes('tvStatic'))drawStatic(context,width,height,.35+high*.65);
 if(effects.includes('heavyStatic')){drawStatic(context,width,height,.78+peak*.22);context.save();context.globalAlpha=.75;context.fillStyle='rgba(255,255,255,.13)';const roll=(performance.now()*.34)%height;context.fillRect(0,roll,width,4+high*18);context.fillStyle='rgba(0,0,0,.18)';for(let i=0;i<6;i++)context.fillRect(0,(roll+i*43)%height,width,1+Math.random()*4);context.restore()}
 if(effects.includes('signalTear')){context.save();context.globalCompositeOperation='screen';for(let i=0;i<4+Math.floor(peak*7);i++){const y=Math.random()*height,h=3+Math.random()*height*.055,offset=(Math.random()-.5)*width*.14;context.fillStyle=`rgba(${Math.random()>.5?255:85},${30+Math.random()*80},${180+Math.random()*75},${.08+peak*.18})`;context.fillRect(offset,y,width,h)}context.restore()}
 if(effects.includes('channelGhost')){context.save();context.globalCompositeOperation='screen';const drift=Math.sin(performance.now()*.0017)*width*.025;context.fillStyle=`rgba(255,35,90,${.06+mid*.09})`;context.fillRect(drift,0,width,height);context.fillStyle=`rgba(20,180,255,${.06+high*.09})`;context.fillRect(-drift,0,width,height);context.strokeStyle='rgba(255,255,255,.09)';for(let y=(performance.now()*.03)%10;y<height;y+=10){context.beginPath();context.moveTo(0,y);context.lineTo(width,y);context.stroke()}context.restore()}
 ...
 if(effects.includes('letterbox')){context.save();context.globalAlpha=1;context.globalCompositeOperation='source-over';const bar=Math.max(18,height*.115);context.fillStyle='rgba(0,0,0,1)';context.fillRect(0,0,width,bar);context.fillRect(0,height-bar,width,bar);context.restore()}
 if(effects.includes('ultrawideBars')){context.save();context.globalAlpha=1;context.globalCompositeOperation='source-over';const bar=Math.max(28,height*.18);context.fillStyle='#000';context.fillRect(0,0,width,bar);context.fillRect(0,height-bar,width,bar);context.restore()}
 if(effects.includes('animeBars')){context.save();context.globalAlpha=1;context.globalCompositeOperation='source-over';const pulse=.07+peak*.09,bar=height*pulse,slant=width*.055;context.fillStyle='#000';context.beginPath();context.moveTo(0,0);context.lineTo(width,0);context.lineTo(width-slant,bar);context.lineTo(slant,bar);context.closePath();context.fill();context.beginPath();context.moveTo(slant,height-bar);context.lineTo(width-slant,height-bar);context.lineTo(width,height);context.lineTo(0,height);context.closePath();context.fill();context.restore()}
 if(effects.includes('retroCinemaBars')){context.save();context.globalAlpha=1;context.globalCompositeOperation='source-over';const bar=Math.max(22,height*.13);context.fillStyle='#030303';context.fillRect(0,0,width,bar);context.fillRect(0,height-bar,width,bar);context.strokeStyle='rgba(255,245,220,.14)';context.lineWidth=2;context.strokeRect(4,bar+4,width-8,height-bar*2-8);context.restore()}
 ...
 if(effects.includes('digitalRain')){
  context.save();context.font='12px monospace';context.fillStyle=`rgba(90,255,190,${.22+high*.42})`;
  for(let i=0;i<34;i++){
   const x=i/34*width,y=(time*(42+i%9)*3+seededValue(i)*height)%height;
   context.fillText(String.fromCharCode(0x30A0+Math.floor(seededValue(i+7)*80)),x,y);
  }
  context.restore();
 }
 if(effects.includes('snowNoise')){
  context.fillStyle=`rgba(255,255,255,${.18+high*.42})`;
  for(let i=0;i<95;i++){
   const x=seededValue(i*3)*width,y=(seededValue(i*5)*height+performance.now()*.015*(4+i%9))%height,r=.5+seededValue(i)*2.4;
   context.beginPath();context.arc(x,y,r,0,Math.PI*2);context.fill();
  }
 }
 if(effects.includes('horizonGlow')){
  const y=height*(.68-bass*.09),gradient=context.createLinearGradient(0,y-32,0,y+32);
  gradient.addColorStop(0,'rgba(140,50,255,0)');
  gradient.addColorStop(.5,`rgba(220,170,255,${.42+peak*.5})`);
  gradient.addColorStop(1,'rgba(80,210,255,0)');
  context.fillStyle=gradient;context.fillRect(0,y-32,width,64);
 }
 if(effects.includes('circleSpectrum')){
  context.save();context.translate(width/2,height/2);context.lineWidth=2;
  const bins=Math.max(1,dataArray?.length||1);
  for(let i=0;i<64;i++){
   const value=(dataArray?.[Math.floor(i/64*bins)]||128)/255,radius=Math.min(width,height)*.18;
   context.rotate(Math.PI*2/64);
   context.strokeStyle=`rgba(${120+i*2},${80+Math.min(175,i*3)},255,${.28+value*.68})`;
   context.beginPath();context.moveTo(radius,0);context.lineTo(radius+value*Math.min(width,height)*.2,0);context.stroke();
  }
  context.restore();
 }
 if(effects.includes('gridFlash')&&peak>.28){
  context.strokeStyle=`rgba(180,100,255,${.18+peak*.5})`;context.lineWidth=1;
  const size=Math.max(28,Math.round(Math.min(width,height)/10));
  const offset=(performance.now()*.03)%size;
  for(let x=-size+offset;x<width;x+=size){context.beginPath();context.moveTo(x,0);context.lineTo(x,height);context.stroke();}
  for(let y=-size+offset;y<height;y+=size){context.beginPath();context.moveTo(0,y);context.lineTo(width,y);context.stroke();}
 }
 if(effects.includes('chromaticPulse')){
  const now=performance.now()*.00012;
  for(let i=0;i<3;i++){
   context.strokeStyle=[
    `rgba(255,40,120,${.18+bass*.48})`,
    `rgba(60,220,255,${.18+mid*.46})`,
    `rgba(180,80,255,${.18+high*.46})`
   ][i];
   context.lineWidth=3;
   context.beginPath();
   context.arc(width/2,height/2,(40+i*35+now*Math.max(width,height)*.22)%(Math.min(width,height)*.48),0,Math.PI*2);
   context.stroke();
  }
 }
 context.restore();
...
function previewLoop(){
 ...
 function draw(now){
  ...
  const effects=selectedEffects(),rawLevels=energy(),levels=video.paused?{bass:.42,mid:.36,high:.3,peak:.42}:rawLevels;recordWavePoint(Number(video.currentTime||0),levels,'exact-audio');applyVideoCss(levels,effects);context.clearRect(0,0,width,height);drawEffects(context,width,height,levels,effects,false);
...
async function render(){
 ...
 recorder.onstop=()=>{updateLiveWaveformStatus('waiting');const blob=new Blob(chunks,{type:mime}),anchor=document.createElement('a');anchor.href=URL.createObjectURL(blob);anchor.download=`sos-video-fx-${Date.now()}.${mime.includes('mp4')?'mp4':'webm'}`;anchor.click();setTimeout(()=>URL.revokeObjectURL(anchor.href),1800);setStatus(`Render complete: ${anchor.download}`)};
 const effects=selectedEffects();video.currentTime=start;await video.play();updateLiveWaveformStatus('recording');recorder.start(250);setStatus(`Rendering ${mime.includes('mp4')?'MP4':'WEBM'} with ${effects.length} effects… Keep this tab open.`);
 function frame(){
  if(video.currentTime>=end||video.ended){video.pause();recorder.stop();return}
  const levels=energy(),{bass}=levels,power=strength();recordWavePoint(Number(video.currentTime||0),levels,'exact-audio');context.save();
...
function convertEntireAudioToWav(){
 if(sourceMode!=='native'||!audioBuffer)return setStatus('Upload or load the original editable media first.');
 downloadExactWav();
}
function convertVideoWithEffects(){
 if(sourceMode!=='native')return setStatus('YouTube preview cannot be converted. Supply the original file or a direct editable video URL.');
 render();
}
...
if(converterSource)converterSource.onclick=downloadSource;
if(converterWav)converterWav.onclick=convertEntireAudioToWav;
if(converterVideo)converterVideo.onclick=convertVideoWithEffects;
...
document.addEventListener('change',event=>{if(event.target.matches('[data-effect]')){updateCount();setTimeout(showPreviewForEffect,20)}});
...
clearYouTubeProjects?.addEventListener('click',event=>{event.preventDefault();clearRecentYouTube()});
playSelectedYouTube?.addEventListener('click',event=>{event.preventDefault();playCurrentYouTube()});
captureSelectedYouTube?.addEventListener('click',event=>{
 event.preventDefault();
 if(!youtubeUrl)return setStatus('Load a YouTube project first.');
 const id=youtubeId(youtubeUrl);updateSelectedCaptureProject(youtubeUrl,id);
 document.getElementById('selectedCaptureProjectV41710')?.scrollIntoView({behavior:'smooth',block:'center'});
 setStatus('The selected YouTube project is now visible in the capture section. Open its playback tab, then choose that tab and enable Share tab audio.');
});
recentProjects?.addEventListener('click',event=>{
 const remove=event.target.closest('[data-delete-recent-youtube]');
 if(remove){event.preventDefault();event.stopPropagation();deleteRecentYouTube(remove.dataset.deleteRecentYoutube);return}
 const button=event.target.closest('[data-use-recent-youtube]');
 if(!button)return;
 event.preventDefault();$('#videoUrlV4171').value=button.dataset.useRecentYoutube;loadUrl();
});
document.addEventListener('click',event=>{
 const control=event.target.closest('#videoEffectsStudio button,#videoEffectsStudio summary,#videoEffectsStudio a[href="#"]');
 if(!control)return;
 if(control.matches('a[href="#"]'))event.preventDefault();
 if(control.matches('[data-effect-preset],[data-effect-clear]')||control.closest('.effectToggleV4171')){
  event.preventDefault();
  setTimeout(showPreviewForEffect,20);
 }
 if(control.matches('button'))setTimeout(()=>control.blur(),0);
},true);
document.addEventListener('click',event=>{
 const preset=event.target.closest('[data-effect-preset]');
 if(preset){event.preventDefault();event.stopPropagation();applyPreset(preset.dataset.effectPreset);setStatus(`${preset.textContent.trim()} preset applied with ${selectedEffects().length} effects.`);setTimeout(showPreviewForEffect,20);return}
 const clear=event.target.closest('[data-effect-clear]');
 if(clear){event.preventDefault();event.stopPropagation();$$('[data-effect]').forEach(input=>input.checked=false);updateCount();setStatus('All visual effects cleared.')}
});
if(waveformDataDownload)waveformDataDownload.disabled=true;if(guideWavDownload)guideWavDownload.disabled=true;if(exactWavDownload)exactWavDownload.disabled=true;updateConverterState();renderRecentYouTube();setReady('video',false);setReady('waveform',false);setCompatibility('isWaiting','Waiting for a video','Direct MP4/WEBM files support full editing. YouTube links support embedded preview only.');updateCount();
let accessAttempts=0;
function resolveStudioAccess(){
 accessAttempts+=1;
 const opened=bootAccess();
 if(!opened&&accessAttempts<20)setTimeout(resolveStudioAccess,250);
}
window.addEventListener('sos:session',()=>{accessAttempts=0;resolveStudioAccess()});
window.addEventListener('sos:profile-updated',()=>{accessAttempts=0;resolveStudioAccess()});
resolveStudioAccess();
